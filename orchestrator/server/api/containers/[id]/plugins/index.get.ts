...
import { usePluginDefinitionStore, usePluginInstallationStore } from "../../../../utils/services";

export default defineEventHandler((event) => {
  const worker = requirePluginWorker(event, getRouterParam(event, "id")!);
  return usePluginInstallationStore().listForWorker(worker.userId, worker.id).map(item => ({ ...item, displayMode: usePluginDefinitionStore().getById(item.definitionId)?.manifest.resources?.display?.mode ?? "none" }));
});
