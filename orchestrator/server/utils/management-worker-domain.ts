...
export class ManagementWorkerDomain {
  ...
  async execute(name: string, args: Record<string, unknown>): Promise<{ handled: boolean; result?: unknown }> {
    ...
    if (name === "workers.clone") {
      ...
      const clone = await cm.create({ userId: worker.userId, displayName: optionalString(args.displayName) || `${worker.displayName || "worker"} copy`, repos: worker.repos, mounts: worker.mounts, hardwareDeviceIds: worker.hardwareDeviceIds, targetWorkerGroupId, environmentId: worker.environmentId, workerSelfApiAccess: worker.workerSelfApiAccess, initScript: worker.initScript, workerConfiguration:{variables} });
      try {
...
function settings(a:Record<string,unknown>):UpdateContainerSettingsRequest { const result:any={}; for(const k of ["displayName","environmentId","initScript","repos","mounts","hardwareDeviceIds"]){if(a[k]!==undefined)result[k]=a[k];} if(a.workerSelfApiAccess!==undefined)result.workerSelfApiAccess=parseWorkerSelfApiAccess(a.workerSelfApiAccess); if(a.excludedGlobalEnvVarKeys!==undefined)result.excludedGlobalEnvVarKeys=strings(a.excludedGlobalEnvVarKeys,"excludedGlobalEnvVarKeys"); if(a.excludedGroupEnvVarKeys!==undefined)result.excludedGroupEnvVarKeys=strings(a.excludedGroupEnvVarKeys,"excludedGroupEnvVarKeys"); return result; }
