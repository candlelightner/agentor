...
export class WorkerGroupEnvStore {
  ...
  private mutate<T>(userId:string,operation:()=>Promise<T>):Promise<T>{const prior=this.queues.get(userId)??Promise.resolve();const result=prior.catch(()=>undefined).then(async()=>{await this.load(userId);return operation();});const tail=result.then(()=>undefined,()=>undefined);this.queues.set(userId,tail);void tail.finally(()=>{if(this.queues.get(userId)===tail)this.queues.delete(userId);});return result;}
}
