...
- **~1386 tests** across 102 test files (~843 API + ~543 UI)
...
tests/
  ...
  api/                     # API endpoint tests (~843 tests across 58 files)
  ui/                      # UI interaction tests (~543 tests across 44 files)
```

## Test Categories

### API Tests (~843 tests, 58 files)
...
| `kilo-code.spec.ts` | 8 | Kilo Code code-server extension + shared per-user config/auth + per-worker code-server data: `code-server --list-extensions --show-versions` reports `kilocode.kilo-code@7.4.16`; the extension resolves to the image-level extension dir; two same-user workers share writes under canonical `~/.config/kilo` (per-user bind) and `~/.local/share/kilo/auth.json` (per-user `credentials/kilo.json` bind); code-server user-data markers are per-worker (NOT shared between same-user workers); cross-user isolation (a second user's worker cannot see the first user's Kilo config/auth); shared Kilo config + per-worker code-server marker survive rebuild |
...
| `account-credentials.spec.ts` | 5 | Per-user agent OAuth credential file listing + reset: auth gating, 4-entry shape for a fresh user (claude/codex/gemini/kilo), idempotent DELETE, unknown agent id rejection, independent listings per user |
...
| `apps-pane.spec.ts` | 10 | Apps pane for container, plus VS Code Tunnel and SSH singleton app types render with `Start` buttons and "Not running" empty state |
