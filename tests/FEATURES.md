...
- Per-user worker groups can be created, renamed, and assigned workers owned by the same user. The dashboard’s **Worker groups** action provides membership editing; the API is `/api/worker-groups`. On the Workers and Stopped tabs, each group’s visible members and its group-admin runtime render together inside a labeled bordered enclosure; the admin runtime is not counted as an ordinary member, and ungrouped workers remain individual cards. The platform-wide ADMIN / ORCHESTRATOR card uses a distinct red privileged background. Changing membership immediately reconciles every group-scoped managed network, with worker protection locks enforced across the union of workers being detached and attached. Deleting a group never deletes its workers, but an in-use group returns 409 until its managed networks are deleted or reconfigured.
...
- Each group-admin MCP has a private image-catalog category keyed by both owner and group identity. It can create, replace, validate, build/rebuild, inspect build status/logs, cancel, promote, roll back, and delete its own definitions and versions. Existing owner/global images remain usable as read-only worker bases, but global and other-group definitions/builds are absent from results and direct access returns the same 404 as an unknown ID. Group images cannot become owner/system defaults or participate in Git catalog synchronization. Evaluation workers may use global or same-group images and are still atomically enrolled in the bound group. A group with private image definitions cannot be deleted until those definitions are removed.
- The group admin's MCP identity is limited to the group's live same-owner members. It delegates member inspection/metrics, lifecycle, configuration/locks, console/logs, running and offline files, exports, backups, apps, and creation of mappings that explicitly name a member. It can create an evaluation worker only through an identity-derived path that enrolls and network-reconciles the new worker into its bound group before returning it. Discovery and list results omit non-members; known out-of-group and unknown target identifiers both fail promptly with the same generic 404. Membership additions grant and removals revoke access immediately.
- Platform/global operations are deliberately excluded: group and membership mutation, worker creation/clone, imports, image/catalog management, managed networks, global configuration/policy, and mapping list/delete. Group-admin credentials, linked console sessions, and one-use download handoffs are workspace- and group-bound. Group deletion or membership removal revokes affected authority, including redemption of already prepared downloads; a token from one group workspace cannot be replayed by another. Normal capability-policy and protection-lock checks still apply.
