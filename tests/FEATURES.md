...
- **User deletion cleanup**: a background `OrphanSweeper` runs at orchestrator startup and every 10 minutes; it reads the auth DB's user table and prunes any per-user `env-vars.json` row, `<DATA_DIR>/users/<userId>/` directory, and usage-state entry whose user is no longer present. Before deleting any owner state it invokes fail-closed mutation barriers; backup cleanup aborts active restores and either drains them within a bounded deadline or defers the entire owner cleanup until a later sweep. Implemented as a timer rather than a middleware so nothing touches better-auth's request pipeline.
