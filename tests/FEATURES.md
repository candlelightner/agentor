...
- Workspace-only is the default; root-filesystem capture is an explicit warned advanced mode. Current v2 bundles preserve Docker's root filesystem as uncompressed `rootfs.tar` for speed; imports also accept legacy v1 `rootfs.tar.gz` bundles. The prior long compression timeout was a historical v1 failure mode, not a current behavior claim. Both paths exclude credentials and secret values.
...
- The management MCP covers worker lifecycle/configuration/groups/locks/console, bounded running-workspace file management, offline workspace browsing and exports, image builds/catalog recovery, backups/restores, controlled networks, port/domain mappings, worker apps, storage cleanup, and user configuration catalogs. Running-file mutations reuse the hardened container file manager, require a protected worker's lock password, and cap a single base64 upload at 1 MiB. Its parity matrix documents intentional non-retrieval of secrets, metadata-only logs, streaming download boundaries, and the current lack of a binary worker-import tool.
...
- Bundle layout (outer uncompressed v2 tar): `manifest.json` + `workspace.tar.gz` + `agents.tar.gz`, and only when explicitly requested, `rootfs.tar` (the uncompressed Docker-export tar). Preserving that stream avoids the historical v1 compression/decompression bottleneck, but can use substantially more temporary/artifact storage; the advanced UI warns before it starts.
