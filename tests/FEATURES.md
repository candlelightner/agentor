...
- Each ordinary worker and group has a live orchestrator-side worker-self API policy: `inherit`, `allow`, or `deny`. A worker override wins, otherwise the nearest explicit ancestor wins, with a backward-compatible default of allow; malformed hierarchies and any denied branch in a legacy overlapping membership fail closed. GUI, REST, and management MCP expose both the stored value and effective decision. Policy changes apply to the next request without restart/rebuild, while already accepted streams are not terminated. Group moves affect this gate immediately too, but may independently require a rebuild when they change immutable host-mount/runtime configuration. Platform/group administrative workspaces retain their narrow scoped management/plugin MCP exception.
- The dashboard’s **Worker groups** action renders and edits the nested tree. On the Workers and Stopped tabs, each group’s direct workers and group-admin runtime render in its own labeled bordered enclosure nested under its parent; ungrouped workers remain individual cards. Group headers distinguish the current tab's direct cards from direct archived members, and archive-only group trees remain visible on the Workers tab with descendant summaries while their cards stay in Archived. REST list/get and MCP `groups.list` expose the same additive direct `memberCounts`. The sidebar periodically refreshes group membership alongside worker inventory, so MCP-created or externally reassigned workers move into the correct enclosure without a page reload. Changing membership or reparenting reconciles every affected ancestor group network, with worker protection locks enforced for detached and attached workers and rollback on reconciliation failure. Permanently deleting a worker atomically removes all of its direct group references and reconciles affected descendant/ancestor networks; authorized group deletion also prunes legacy references whose durable worker records are already absent, while live and archived workers remain blockers. Deleting a group never deletes workers and returns 409 while it has children, direct workers, private images, or managed-network references.
- A group header exposes icon-only **Stop all**, **Rebuild all**, and **Archive all** actions for every ordinary worker in that group and all descendants. All affected protection locks are preflighted before the first lifecycle mutation; group-administrative workspaces use their separate lifecycle and are never included. REST and management MCP return per-worker success/skip/failure results, reject overlapping owner batches promptly, and bound MCP caller waits without allowing timeout retries to enqueue duplicate work. Archive preserves direct group membership, and the Archived sidebar reconstructs the same nested group enclosures so individual unarchive returns workers to their original visual group.
- A group owner can provision one trusted **group admin workspace** from the group's controls and manage it through `GET`/`POST /api/worker-groups/:id/admin-workspace` plus `POST /api/worker-groups/:id/admin-workspace/{start,stop,rebuild}`. It has the standard trusted admin terminal/editor/desktop runtime, host-to-noVNC image/text clipboard bridge, private management-MCP bridge, and the same symlink-safe `/workspace` Files picker as ordinary workers. The platform-wide administrator workspace also exposes that Files picker from its privileged services panel. Admin file access uses the existing authenticated container-file routes and remains scoped to an authorized owner or platform administrator. Both admin kinds have a persistent, non-secret startup script editor: saving is non-disruptive and marks the immutable runtime configuration pending; the next explicit start or rebuild recreates only disposable compute, then launches the script in the admin tmux pane on every start. Desired/applied revisions and bounded running/succeeded/failed metadata are visible without logging script contents; `/workspace` and agent data survive. Ordinary worker-inventory refreshes preserve its external service registration, so editor/desktop readiness and proxy routes remain available after rebuild and subsequent dashboard polling. Concurrent lifecycle requests are never queued: while one operation is active, later GUI/MCP retries fail promptly with HTTP 409 instead of scheduling repeated container recreation.
- Each group-admin MCP has a private image-catalog category keyed by both owner and group identity. It can create definitions for its bound group or descendants and manage definitions owned within that subtree. Global and ancestor definitions are visible/use-only; sibling and unrelated definitions/builds are absent, and direct access returns the same 404 as an unknown ID. Group images cannot become owner/system defaults or participate in Git catalog synchronization. Evaluation workers may use global or inherited ancestor images and are atomically enrolled in the selected authorized group. A group with private image definitions cannot be deleted until those definitions are removed.
- A group admin's MCP identity covers its bound group and live descendant subtree. Ancestor admins can create/reparent/delete descendant groups, move workers already inside their subtree, manage descendant admin workspaces (including their startup scripts) and group-scoped networks, and operate descendant resources; the global admin can manage every group-admin script. Identity-bound `admin-workspace.startup-script.{get,set}` never accepts a target ID, while `groups.admin-workspace.startup-script.{get,set}` validates its `groupId` against the live subtree and fails fast. Group admins cannot move/delete their own authority root, ungroup workers, import sibling/ungrouped workers, or inspect parents/siblings/unrelated branches. It also delegates worker inspection/metrics, lifecycle, configuration/locks, console/logs, running and offline files, exports, backups, apps, and mappings. Evaluation workers default to the bound group or an authorized descendant and are enrolled before return. Discovery omits unauthorized resources; known foreign and unknown identifiers both fail promptly with the same generic 404. Membership or reparenting grants/revokes access immediately, including inside queued mutations, open sessions, and prepared downloads.
- Worker environment inheritance is names-only at every read boundary. A worker can exclude selected predefined/configured account variables and effective group variables; exclusions persist and apply on create/rebuild without exposing values. Groups store encrypted write-only variables, inherited root-to-leaf with nearer overrides; subgroups may exclude effective parent keys before adding their own. Group values are delivered through the existing tmpfs secret-handshake path rather than Docker `Config.Env`, so `docker inspect` does not expose their plaintext. Group admins may manage variables only in their authorized subtree. Key changes mark affected workers pending rebuild, moves/reparenting recalculate inheritance, stale exclusions are ignored safely at runtime, and group deletion transactionally removes/restores encrypted records and the admin runtime if final persistence fails.
- Installation-wide operations remain excluded: owner-global configuration/policy, imports, global/default image mutation, Git catalog synchronization, owner-wide network scopes, and mapping list/delete. Group-admin credentials, linked console sessions, and one-use download handoffs are workspace- and group-bound. Group deletion or membership removal revokes affected authority, including redemption of already prepared downloads; a token from one group workspace cannot be replayed by another. Normal capability-policy and protection-lock checks still apply.

## Managed worker networks

- Owner-scoped managed bridge networks attach all, selected, or group-derived workers through the orchestrator Docker boundary. Group-derived membership follows group edits in actual Docker topology. Management/internal networks are rejected; topology and membership validation are available through `/api/managed-networks`.

## Worker protection locks

- A worker can have an optional separate password-based protection lock. The lock password is a one-way verifier and is never returned. While protected, every material mutation requires the correct write-only `lockPassword`, including settings and worker-local configuration, running-workspace file changes, app and exposure-mapping changes, start/stop/restart, archive/unarchive, rebuild, in-place backup restore, and permanent deletion. Creating a separate new worker from a backup does not mutate the protected source and remains available without its password. The Worker Settings and running Files dialogs accept the credential only in password inputs, and `/api/containers/:id/protection` supports state, set/change, remove, and verify operations.
- The lock covers Agentor control-plane mutations, not commands run through a terminal/console by a caller who already has console authority. Console access continues to use its ownership and MCP capability boundaries; Agentor does not claim to intercept or classify arbitrary commands inside the worker.
...
- **Agent OAuth credentials section**: per-agent (claude/codex/gemini/kilo) status indicator (`Logged in` green / `Not logged in` gray) plus a per-agent **Reset** button (two-step confirm) that overwrites the OAuth file with `{}` so the next CLI login writes fresh tokens. Login itself happens by running the agent CLI inside one of the user's workers — credentials live at `<DATA_DIR>/users/<userId>/credentials/` (claude/codex/gemini) and are bind-mounted into all of the user's workers. Kilo's auth now lives in the shared per-user data directory (`<DATA_DIR>/users/<userId>/kilo/data/auth.json`); Account Reset targets that live shared auth file. Kilo is a fourth Account credential/reset row alongside Claude/Codex/Gemini, but it has no usage-monitoring endpoint and no separately installed Kilo CLI — the orchestrator only manages its shared config/auth/data files.
...
### 4.3 Action Buttons
- "Stop" — visible when running, stops the container. The control-plane stop
  operation is idempotent: repeating it for an already-stopped worker succeeds
  and leaves the worker stopped.
- "Restart" — visible when stopped, restarts the container
- "Recover worker" — visible for an `unknown` runtime state, with a tooltip explaining that Agentor could not verify the task; a confirmation spells out the persistence guarantees, protected workers request their transient lock password only after a server-validated 423, and success refreshes the card and opens the recovered terminal.
...
- **Persistent VS Code** (`vscode-desktop`) — singleton (`maxInstances: 1`), no container port, no exposed external port. Apps → **Persistent VS Code** launches a Chromium **app-mode** client against the local code-server (already running on port 8443) under the existing Xvfb/noVNC display stack and automatically opens **Desktop** on success. It uses a persistent browser profile stored in the agent-data volume so browser cookies/preferences survive restarts; code-server extension/UI state persists separately in its per-worker user-data directory. It reuses code-server + the existing Chromium — no Kilo source is patched and no desktop VS Code package is installed.
...
### 19.4 Persistent VS Code client (singleton app) lifecycle
- Apps → **Persistent VS Code** starts a Chromium app-mode client pointed at the local code-server (port 8443) under the Xvfb/noVNC stack. It exposes **no external port**; a successful Start automatically opens the **Desktop** pane, while failures are surfaced as a dashboard toast. The client's persistent profile lives in the agent-data volume.
...
- Opt-in `resources.display.mode: isolated` creates an installation-owned Xvfb/fluxbox session with bounded dimensions and native `kind: desktop` actions. Authenticated noVNC panes/tabs use installation-bound Docker-exec RFB streams with no listening VNC/websockify port; concurrent displays, refresh/reconnect, startup/crash reconciliation, and scoped disable/uninstall preserve the normal `:99` desktop. Inspection exposes safe mode/allocation/readiness/error state. Viewer authorization binds user, worker, installation, definition, action and display, with live revocation. Plugins sharing a sudo-capable worker user remain within the same process trust boundary.
...
- Private UI actions are authenticated, worker-scoped, sandboxed dashboard panes proxied only to a declared plugin port/path. A `desktop` action mode is additionally available for noVNC: after the same installation/action authorization, it redirects into Agentor's authenticated `/desktop/:id/agentor.html` clipboard-aware route. Neither mode creates a public port or Traefik mapping.
- An ordinary worker, the global administrative workspace, and a group-administrative workspace can use the narrow worker-self plugin MCP only for plugins on that exact runtime. Identity is derived from its live Docker source-IP/network placement (the singleton `agentor-management` network or the bound `agentor-management-group-*` network), authoritative external-runtime registration, labels, and role—not request fields or runtime environment variables. This plugin-only exception never grants administrative workspaces generic worker-self port, domain, usage, or info access.
- A global admin may use platform and its own workspace-scoped definitions. A group admin may use platform, bound-group/ancestor-group, and its own workspace-scoped definitions, but never owner-wide, sibling, descendant-only, other-admin, or unrelated-group definitions. Self-created definitions are always scoped to the exact runtime. Management MCP remains required for authorized cross-worker actions.
- Worker-self plugin support is delivered by the worker image and trusted-admin overlay. Existing immutable custom-image versions need a new version based on a current Agentor base before a rebuild can gain it; an administrative workspace receives it after its updated overlay is applied by start/rebuild. Existing desired installations and user-owned files persist across that reconciliation, including managed worker recovery, which refreshes observed runtime generation before re-evaluating readiness.
- The additive custom-image catalog normalizes legacy definitions to Safe provisioning without a separate database migration. Safe-mode rejection is actionable and happens before Docker; Advanced allows arbitrary build-time shell only within Agentor's controlled build boundary. Durable build, compatibility-validation retry, and test-worker jobs share status/log/cancel handling and requestId idempotency. A version must be compatibility `ready` or `ready-with-warnings` before promotion or a test worker; `built-incompatible` and `validation-unavailable` remain distinct outcomes. Plugin `imageBuild` contributes only secret-free build inputs and validation, never runtime state, resources, configuration, or secrets.
...
- The archive is streamed with backpressure; an early client disconnect tears down the underlying streams and cancels bounded Docker helper setup. Timeout/abort responses use safe structured diagnostics, and stale helpers are cleaned independently so one timeout does not block later downloads. No host path is ever exposed (the Docker `getArchive` tar is consumed and only the file/ZIP payload is emitted).
...
- The injected `agentor-clipboard.js` module capture-intercepts Ctrl/Cmd+V (and Ctrl+Alt+V) in the noVNC iframe BEFORE noVNC sends the key to the remote VNC server. On a real user gesture (display focused) it reads the host clipboard, normalises images to PNG, POSTs to `POST /api/containers/:id/clipboard` (with credentials), then replays Linux Ctrl+V to the VNC session via `UI.rfb.sendKey` (numeric X11 keysyms + physical key codes) — translating host Cmd+V correctly and only after the 200 (the helper verified X selection ownership). The worker helper targets the display actually exported by x11vnc, with validated configured/live-Xvfb fallbacks instead of assuming `:99`. On denied/error it replays Ctrl+V once to preserve paste behaviour. Concurrent pastes are guarded.
...
- Persisted account/worker configuration is validated as one owner-partition snapshot. Malformed JSON, invalid records, duplicate IDs, aggregate-size violations, or an embedded owner mismatch quarantine only that owner with a retry-safe 503; reads and writes cannot expose a stale live reference or overwrite/drop the original corrupt bytes. Explicit deleted-owner cleanup may remove the quarantined file.
...
- `POST /api/containers` — create worker. Body NO LONGER accepts `id` (or `name`). Accepts optional `displayName` (string, trimmed, ≤100 chars else HTTP 400), `workerGroupId`, environmentId, initScript, repos, governed mounts, and governed `hardwareDeviceIds`. The orchestrator mints the UUID `id` itself; the response `ContainerInfo.id` is that UUID, `.containerId`/`.containerName`/`.imageName` are populated, and `.displayName` is always populated (defaults to an adjective-animal slug when omitted). `workerGroupId` must be a same-owner group and enrollment/network reconciliation is part of the create contract. **Host mounts use `{ pathId, target, readOnly }`**: only a platform admin can enter the raw canonical source into the empty-by-default catalog, the account must be entitled, and an owner all/group/worker assignment must apply. The server resolves source by `pathId`, read-only is the default, and writable additionally requires catalog `allowWrite`; colon/traversal/system/Docker/Agentor-data paths are rejected.
- `PATCH /api/containers/:id` — update editable worker settings WITHOUT recreating the container (`:id` is the worker UUID). Body is a partial `{ displayName?, environmentId?, initScript?, repos?, mounts?, hardwareDeviceIds? }` — only the present keys change. Returns the updated `ContainerInfo`. The internal identity (`id`, `containerName`, volumes, mappings) is always unchanged; `updatedAt` is bumped.
...
- `POST /api/containers/:id/rebuild` — rebuild worker (destroys and recreates with latest image, preserves workspace, agents, and DinD volumes plus authorized host bind metadata — equivalent to archive + unarchive). Every mount is re-resolved through the current catalog/grant state. Applies any `pendingRebuild` settings edits and clears `hostMountsRevoked` only after the replacement container no longer carries a revoked bind.

### 24.2a Governed host mounts

- `admin/host-mount-paths.v1.json` is the versioned platform path catalog; `users/<id>/host-mount-grants.json` stores entitlements, owner assignments, and downward delegation ancestry. Both start effectively empty for a new installation.
- Platform REST/MCP manages catalog, account entitlement, and owner all/direct-group/worker assignments. Group management MCP exposes only `host-mounts.delegations.list/create/delete`, derives its authority group from workload identity, separates worker-usable `availablePaths` from explicitly granted `delegablePaths`, omits raw host sources, and validates every target downward. Worker MCP schemas never accept raw `source`.
- Deleting a path/grant/entitlement, disabling required write access, or changing hierarchy runs one reconciliation path. Unauthorized desired mounts are removed, affected running workers stop, and persisted `hostMountsRevoked` blocks restart. Rebuild removes the Docker bind and clears the guard; startup retries a previously failed stop.
- Custom-image provisioning does not grant a host source, Docker socket, or full-Dockerfile escape. See `docs/host-mounts.md`.

### 24.2b Governed hardware devices

- The **Hardware** dialog discovers live DRM GPUs and USB devices through an isolated read-only, networkless, capability-free helper. Platform approval stores stable PCI or USB identity rather than volatile `/dev` numbering.
- Account entitlement plus an owner all/direct-group/worker assignment is required. Group MCP can delegate only an explicit group grant to descendants; all worker APIs and MCP use opaque `hardwareDeviceIds` and never accept raw device nodes.
- Create/rebuild resolves current nodes into Docker `HostConfig.Devices` and `GroupAdd`. Unavailable hardware fails explicitly. Revocation removes desired IDs, stops live workers, persists `hardwareDevicesRevoked`, blocks restart, and requires rebuild.
- Catalog and grant ancestry live in `admin/hardware-devices.v1.json` and `users/<id>/hardware-device-grants.json`. See `docs/hardware-devices.md`.
...
- `POST /api/containers/:id/apps/:appType` — start instance. Returns `{ id, port, externalPort? }`. Returns HTTP **409** when starting a singleton (`vscode` / `vscode-desktop` / `ssh`) that already has a running instance. For `vscode` the orchestrator passes the per-user tunnel name as an extra arg to `manage.sh start`; for `ssh` the orchestrator allocates an external port in `22000–22999` (reusing any existing mapping for `(containerName, appType='ssh', instanceId='ssh')`), writes it to the port-mapping store with `appType: 'ssh'`, `instanceId: 'ssh'`, and triggers a Traefik reconcile. For `vscode-desktop` the app launches a Chromium app-mode client against the local code-server (no exposed port).
...
- `GET /api/containers/:id/metrics` — a single worker's `WorkerMetrics` (see §24.2). `WorkerMetrics` = `{ workerId, containerName, displayName, status, cpuUtilization (0-100 of host, from docker stats), memoryUsedBytes, memoryLimitBytes, memoryUtilization, diskUsedBytes (bounded durable /workspace + agent-data du; 60s cadence), netRx/TxBytesPerSec, blkRead/WriteBytesPerSec, lastChecked, error? }`.
...
- **Worker provisioning**: when a worker is created/rebuilt/unarchived, the orchestrator resolves the worker owner's `UserEnvVars` and bind-mounts the user's three OAuth credential files directly at the CLIs' expected paths — `/home/agent/.agent-data/.claude/.credentials.json`, `/home/agent/.agent-data/.codex/auth.json`, `/home/agent/.agent-data/.gemini/oauth_creds.json`. For Kilo the orchestrator bind-mounts the **complete** per-user data directory `<DATA_DIR>/users/<userId>/kilo/data` at `/home/agent/.agent-data/.kilo/shared-data` (a **directory** bind, not a file bind) so Kilo's login, provider API keys, and SQLite sessions/history are shared per user across all that user's workers — Kilo atomically temp+renames `auth.json`, which would break a file-level bind. The per-user Kilo config directory (`<DATA_DIR>/users/<userId>/kilo/config`) is also bind-mounted over `.agent-data/.kilo/config` so the Kilo global config is shared per user across all their workers. In directory mode, `StorageManager.ensureWorkerDirs` pre-creates the mountpoint files so Docker Desktop's virtiofs accepts the nested file bind. Writes from any of the user's workers update the same host file immediately, with no symlink indirection or restart dance.
...
- **User deletion cleanup**: a background `OrphanSweeper` runs at orchestrator startup and every 10 minutes; it reads the auth DB's user table and prunes every live/archived worker plus per-user stores, `<DATA_DIR>/users/<userId>/`, and usage state whose user is no longer present. Create/import/settings/rebuild/archive/unarchive/delete operations and Git-catalog connection admission share an owner→worker lifecycle fence; after acquiring it they revalidate the auth owner, so queued work cannot resurrect an account or its encrypted Git credential while cleanup removes it. Runtime and storage deletion is fail-closed and retryable: Docker/volume/image/mapping or filesystem failures retain a durable or in-memory worker/owner handle, and the physical user-directory scan remains a restart-safe final candidate source. Backup cleanup first aborts active restores and either drains them within a bounded deadline or defers the entire owner cleanup. Implemented as a timer rather than middleware so nothing touches better-auth's request pipeline.
...
A dedicated set of routes mounted at `/api/worker-self/*` is **public to the auth middleware** (listed in `PUBLIC_API_PREFIXES`) but identifies the calling worker by source IP via `requireWorkerSelf()` (`server/utils/worker-auth.ts`). The orchestrator joins the same `agentor-net` Docker bridge that workers join, so each worker's source IP is unique and can be matched against the `agentor.managed=true` containers. Calls from outside the Docker bridge (e.g. host `localhost:3000`) return 401 because the source IP doesn't resolve to any managed worker. Every ordinary-worker request then resolves the current persisted worker/group policy; denial is a structured 403 (`WORKER_SELF_API_DISABLED`) and applies to the complete worker-self surface, including plugin MCP, immediately without container recreation.
...
The port-mapping / domain-mapping / usage routes additionally **enforce the calling worker's environment `exposeApis` flags**: the handler resolves the worker's environment and returns **403** when the relevant flag (`portMappings` / `domainMappings` / `usage`) is false. `info` and the `*-mapper/status` routes bypass only those per-capability flags; a complete worker/group denial still blocks them. Port/domain create/delete/batch mutations also require the calling worker's transient write-only `lockPassword` when it is protected; read-only list/status routes remain available. Disabling an exposure flag blocks the endpoint, not just its capability doc.
...
`POST /api/worker-self/mcp` is a separate narrow JSON-RPC MCP endpoint for plugins installed on the calling runtime. It derives ordinary-worker identity from the Docker bridge source IP; for this endpoint alone, it can also recognize the exact live, labeled, registered platform/group administrative workspace on its expected management network. It implements initialization plus `tools/list` and `tools/call`, cannot target another worker, workspace, owner, or group, and returns bounded sanitized structured errors. This does not change the ordinary-worker-only contract of the other worker-self routes above.
...
#### 26.1.1 Authoritative runtime-role skills

- Agentor installs exactly one built-in role guidance skill on every runtime boot: `agentor-global-administration` for the trusted platform workspace, `agentor-group-administration` for a group-administrative workspace, or `agentor-worker-runtime` for an ordinary worker. The corresponding Claude and Codex `SKILL.md` files and Gemini command are generated from the same built-in payload.
- Privileged role selection comes only from the orchestrator's authoritative container-creation path and administrative workspace record. Account, environment, worker-local, instruction, and capability payloads cannot select a privileged role. Missing or invalid role identity fails closed to ordinary-worker guidance.
- Role reconciliation runs after generic capability setup on every start/rebuild. It updates the one allowed Agentor-owned role entry, removes only the two forbidden reserved role entries, and preserves generic capabilities plus every unrelated user-owned skill and command in persistent agent data.
- These skills are operational guidance, not authorization. The management MCP's server-side platform/group identity, live subtree checks, policy, and protection locks remain the security boundary.
- Each role explains the managed visible-desktop path: launch GUI applications on the runtime's configured `DISPLAY` (normally `:99`), leave Agentor's Xvfb/x11vnc/websockify processes intact, and have the operator create a fresh viewer connection through **Desktop** or **Open in tab**.
...
- Platform administrators can create a separate whole-instance disaster-recovery snapshot. The authenticated `AGENTOR-INSTANCE-BACKUP-1` envelope contains a consistent SQLite online backup, versioned `DATA_DIR` state (including complete plugin definitions and desired installations), a secret-free image/host-mount inventory, and optionally Agentor-owned Docker volumes. Logs and recursive local/export/staging artifacts are excluded by default. Docker image layers, deployment `.env`/PEM credentials, and host bind-mount contents stay explicit external dependencies.
- Instance snapshots can stay local, be downloaded/uploaded, or use the existing optional Google Drive connection. Instance and portable-worker provider markers/queries are disjoint. A fresh installation linked to the same account and `drive.file` OAuth app can idempotently scan, identify missing recovery keys, adopt only after full authentication/integrity/archive validation, and retain failed discoveries for retry. A different OAuth client normally cannot enumerate the prior app's `drive.file` objects.
- Whole-instance restore preflights an empty destination, matching directory/volume storage mode and `CONTAINER_PREFIX`, absent destination volume names, image/external dependencies, and host-mount policy choice. Host-mount policies are omitted by default. Two explicit acknowledgements acquire an acceptance-to-handoff mutation barrier and gate a network-disabled helper that revalidates input, stops the exact orchestrator, independently rechecks persisted worker/admin state and Docker labels, swaps `DATA_DIR`, creates only absent selected volumes, persists terminal job ownership across the restored `auth.db`, restarts the exact target, and rolls back helper-owned changes on failure.
- Instance create/discovery/adoption/upload-verification/restore use the same persisted jobs in GUI, REST, and platform-management MCP. Starts return promptly with job identity and exact status/incremental-log/cancel next actions; stable request identities deduplicate transport retries, every REST start accepts a matching `Idempotency-Key`, and the UI reuses an identity after uncertain delivery. Group administrators cannot discover these platform tools. Normal status surfaces expose recovery-key fingerprints/availability only, never raw material.
...
- The persistent digest-pinned `ADMIN / ORCHESTRATOR` overlay reuses terminal/editor/desktop, persistent storage, red identity markers, and a generated non-secret control representation. It has no raw Docker socket, host binds, privilege, or published ports. Dashboard proposal review is optional; harness confirmation is not a platform security boundary.
- Platform and group administrative workspaces receive distinct, non-user-invocable management guidance selected from their persisted authoritative workspace kind. Neither privileged skill is exposed through the generic capability catalog, neither can enter an ordinary worker, and group guidance explicitly retains the bound-subtree boundary. Updating the administrative overlay replaces only disposable compute; persistent workspace files, user configuration, and unrelated skills remain intact.
- The MCP listens only on an internal management network, is preconfigured in the administrative Codex environment through a rotating-credential stdio bridge, rechecks a fail-closed live group allowlist, omits disabled groups from discovery, audits every success/failure without args/results, and supports bounded/redacted linked-tmux console sessions inside selected workers. Every parsed HTTP MCP failure retains the caller's JSON-RPC id; the stdio bridge also bounds upstream I/O and safely correlates non-success status, empty/malformed JSON, mismatched ids, connection failures, and timeouts so a client request cannot remain pending. It also exposes an owner-checked bounded/redacted Docker-log tail and sanitized read-only usage/worker-metric snapshots. Explicit cross-user `ownerId`/`userId` selectors remain available only to the platform administrative agent, must resolve to a real Better Auth user, and pass the shared traversal-safe user-ID boundary before any user-scoped store or filesystem path is reached. Immutable configuration proposals retain optional dashboard review; the invoking harness controls confirmation while Agentor enforces identity and capabilities.
- Lifecycle timeout and managed-recovery tool errors use structured, allowlisted diagnostics (operation/phase, retryability, preservation state, and a safe next action), never Docker commands, container IDs, or secret-bearing causes.
- The management MCP covers worker lifecycle/configuration/groups/locks/console, bounded running-workspace file management, offline workspace browsing, exports/imports, image builds/catalog recovery, backups/restores, controlled networks, port/domain mappings, worker apps, storage cleanup, user-global plain variables, and user configuration catalogs. Large offline file/directory downloads, completed export artifacts, and worker imports use the private management listener: JSON tools mint short-lived one-use administrative-workspace-bound paths, the current rotating bearer credential is supplied only as an Authorization header, exact owner/resource/path bindings and the live capability are rechecked at redemption, and bodies stream with backpressure/disconnect cleanup instead of session cookies, public routes, URL credentials, or base64 buffering. All aliases that materially mutate a protected worker—including legacy lifecycle/configuration tools, app control, exposure mappings, and running-file mutations—require a transient write-only lock password; mapping deletion resolves the affected worker before verification. Running-file upload is capped at 1 MiB. Global variables are not falsely presented as encrypted secrets: sensitive-looking names are masked on reads, while true write-only secrets remain worker-local. Its parity matrix documents non-retrieval of secrets and binary/streaming boundaries.
- Managed-network updates share an owner-scoped mutation queue and recover desired state plus prior Docker topology after either reported partial failures or thrown Docker errors; an incomplete rollback is surfaced instead of silently leaving UI/API/MCP state divergent.
- Backup startup deterministically fails interrupted queued/running jobs and removes their per-job backup/restore scratch. Conservative stale-staging cleanup also recognizes aged `restore-*` directories.
...
- On every managed start, legacy duplicate GitHub credential helpers converge to the one supported `gh auth git-credential` helper; the operation is idempotent across retries/restarts.
...
- Bundle layout (outer uncompressed v3 tar): `manifest.json` + `workspace.tar.gz` + `agents.tar.gz`, and only when explicitly requested, `rootfs.tar.gz` (parallel level-1 gzip of Docker export). This avoids the historical single-core compression bottleneck while bounding temporary/artifact storage; the advanced UI warns before it starts.
...
- The full-rootfs proof has a dedicated 45-minute single-attempt envelope because safe restore validates the inner archive before Docker creates a new image layer; this measured nested-Docker duration is distinct from the asynchronous export job/UI lifetime and is documented as an advanced-path tradeoff.
...
- Restore steps: resolve/create the environment (built-in reused by id; a user's same-named env reused; otherwise the embedded definition is recreated as a new custom env) → import current v3 or legacy v1 `rootfs.tar.gz`, or transitional v2 `rootfs.tar`, into a per-worker image (`agentor-import-<id>`) when present, replicating the standard image's entrypoint/env so it boots; a captured-rootfs failure remains explicit and retryable rather than silently substituting the standard image → create the container stopped → restore the workspace + agent-data volumes via `putArchive` → start → recreate port/domain mappings (skipping conflicts and base domains not configured on this machine).
...
## 29. Kilo Code (code-server extension + shared config/auth/data)

### 29.1 Preinstalled code-server extension
- The worker image preinstalls the Kilo Code code-server extension (`kilocode.kilo-code`) pinned at **7.4.16** in the image-level code-server extension directory (`/home/agent/.local/share/code-server/extensions`). No per-worker manual install is needed.
- Existing workers do NOT pick up a new/changed extension until the worker image is updated **and** the worker is rebuilt — `code-server --list-extensions --show-versions` reports `kilocode.kilo-code@7.4.16`, and `code-server --locate-extension kilocode.kilo-code` resolves to the image-level extension dir.

### 29.2 Per-user shared Kilo config, auth, & data (across all of a user's workers)
- Kilo global config (`~/.config/kilo`) is **shared per user** across all that user's workers/new workers: the orchestrator bind-mounts the per-user config directory `<DATA_DIR>/users/<userId>/kilo/config` over `/home/agent/.agent-data/.kilo/config`, and the entrypoint symlinks `~/.config/kilo → .agent-data/.kilo/config`. A config write through the canonical path on one worker is visible on every other worker the same user owns.
- Kilo's **complete** `~/.local/share/kilo` data directory is **shared per user** across all that user's workers: the orchestrator bind-mounts the per-user data directory `<DATA_DIR>/users/<userId>/kilo/data` at `/home/agent/.agent-data/.kilo/shared-data`, and the entrypoint symlinks `~/.local/share/kilo → .agent-data/.kilo/shared-data`. This directory includes login (`auth.json`), provider API keys configured through Kilo's VS Code UI, and Kilo SQLite sessions/history — all shared per user across that user's workers/new workers.
- **Why a directory bind is required (not a file bind):** Kilo atomically writes `auth.json` by creating a temp file then renaming it over the target (`temp + rename`). A file-level bind mount replaces only that one file's backing inode, so the atomic rename writes to the host's *original* file rather than the bind target, silently breaking the shared credential. A directory bind wraps the whole directory, so the temp+rename stays inside the shared directory and lands on the shared storage.
- **Migration:** Existing legacy per-user auth at `<DATA_DIR>/users/<userId>/credentials/kilo.json` and any per-worker `.kilo/data` directory migrate on the next worker start/rebuild — missing auth providers merge into the shared directory **without overwriting** entries already present in shared storage, and the first available DB/history file wins when several workers contribute candidates. Account **Reset** targets the live shared auth (it empties the shared `auth.json`, not a per-worker copy).
- Kilo is a **fourth Account credential/reset row** alongside Claude/Codex/Gemini (its shared per-user auth now lives at `<DATA_DIR>/users/<userId>/kilo/data/auth.json`), but it has **no usage-monitoring endpoint** and **no separately installed Kilo CLI** — the orchestrator only manages its shared config/auth/data files. Do not imply Kilo usage monitoring or a standalone Kilo CLI. The Kilo Code experience is delivered via the `kilocode.kilo-code@7.4.16` code-server extension preinstalled in the worker image; no Kilo source was patched and no desktop VS Code package was installed.

### 29.3 Per-worker Kilo state/cache & code-server user-data (NOT shared)
- Kilo **state** (`~/.local/state/kilo`) and **cache** (`~/.cache/kilo`) remain **per-worker** in `.agent-data` and survive restart/rebuild/archive — they are NOT shared across same-user workers. code-server user/UI state likewise stays per-worker (`--user-data-dir $AGENT_DATA/.code-server` in the per-worker agent-data volume).
- code-server runs with `--user-data-dir $AGENT_DATA/.code-server` (the per-worker agent-data volume), so code-server user-data markers written on one worker do NOT appear on another worker owned by the same user.
- Cross-user isolation: a second user's worker cannot see the first user's Kilo config, auth, or shared data, and each worker sees only its own owner's values.

### 29.4 Persistence across lifecycle
- Shared per-user Kilo config and shared data directory (bind-mounted) survive rebuild; per-worker Kilo state/cache and code-server user-data (agent-data volume) survive rebuild. All of these survive restart and archive/unarchive.

### 29.5 Worker export/import exclusion
- Worker export **strips** `.kilo/config`, `.kilo/shared-data`, and the legacy `.kilo/data/auth.json` from `agents.tar.gz` — an export never carries another user's tokens, provider keys, Kilo sessions/history, or account-level Kilo configuration. Per-worker Kilo state/cache and code-server user-data are retained.
