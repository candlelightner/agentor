...
- The management MCP covers worker lifecycle/configuration/groups/locks/console, offline workspace browsing and exports, image builds/catalog recovery, backups/restores, controlled networks, port/domain mappings, worker apps, storage cleanup, and user configuration catalogs. Its parity matrix documents intentional non-retrieval of secrets, metadata-only logs, streaming download boundaries, and the current lack of a binary worker-import tool.
