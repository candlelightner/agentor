...
- The management MCP covers worker lifecycle/configuration/groups/locks/console, bounded running-workspace file management, offline workspace browsing, exports and a one-use private streaming worker-import handoff, image builds/catalog recovery, backups/restores, controlled networks, port/domain mappings, worker apps, storage cleanup, user-global plain variables, and user configuration catalogs. All aliases that materially mutate a protected worker—including legacy lifecycle/configuration tools, app control, exposure mappings, and running-file mutations—require a transient write-only lock password; mapping deletion resolves the affected worker before verification. Running-file upload is capped at 1 MiB. Global variables are not falsely presented as encrypted secrets: sensitive-looking names are masked on reads, while true write-only secrets remain worker-local. Its parity matrix documents non-retrieval of secrets and binary/streaming ceilings.
