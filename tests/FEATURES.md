...
- **Persistent VS Code** (`vscode-desktop`) — singleton (`maxInstances: 1`), no container port, no exposed external port. Apps → **Persistent VS Code** launches a Chromium **app-mode** client against the local code-server (already running on port 8443) under the existing Xvfb/noVNC display stack. It uses a persistent browser profile stored in the agent-data volume so browser cookies/preferences survive restarts; code-server extension/UI state persists separately in its per-worker user-data directory. The user then opens **Desktop** to view and drive it. It reuses code-server + the existing Chromium — no Kilo source is patched and no desktop VS Code package is installed.
...
- Apps → **Persistent VS Code** starts a Chromium app-mode client pointed at the local code-server (port 8443) under the Xvfb/noVNC stack. It exposes **no external port** — it is only reachable by opening the **Desktop** pane. The client's persistent profile lives in the agent-data volume.
