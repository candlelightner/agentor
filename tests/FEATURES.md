...
- **Worker provisioning**: when a worker is created/rebuilt/unarchived, the orchestrator resolves the worker owner's `UserEnvVars` and bind-mounts the user's four credential files directly at the CLIs' expected paths — `/home/agent/.agent-data/.claude/.credentials.json`, `/home/agent/.agent-data/.codex/auth.json`, `/home/agent/.agent-data/.gemini/oauth_creds.json`, and `/home/agent/.agent-data/.kilo/data/auth.json` (Kilo's canonical in-worker path is `~/.local/share/kilo/auth.json`). The per-user Kilo config directory (`<DATA_DIR>/users/<userId>/kilo/config`) is also bind-mounted over `.agent-data/.kilo/config` so the Kilo global config is shared per user across all their workers. In directory mode, `StorageManager.ensureWorkerDirs` pre-creates the mountpoint files so Docker Desktop's virtiofs accepts the nested file bind. Writes from any of the user's workers update the same host file immediately, with no symlink indirection or restart dance.
...
- Per-user OAuth credential files (`.claude/.credentials.json`, `.codex/auth.json`, `.gemini/oauth_creds.json`, `.kilo/data/auth.json`) and the shared `.kilo/config/` directory are **stripped** from `agents.tar.gz` — an export never carries another user's tokens or account-level Kilo configuration.
...
## 29. Kilo Code (code-server extension + shared config/auth)
...
### 29.2 Per-user shared Kilo config & auth (across all of a user's workers)
...
- Kilo OAuth auth (`~/.local/share/kilo/auth.json`) is **shared per user** across all that user's workers: the per-user auth file `<DATA_DIR>/users/<userId>/credentials/kilo.json` is bind-mounted at `/home/agent/.agent-data/.kilo/data/auth.json` (surfaced at `~/.local/share/kilo/auth.json` via the `.local/share/kilo → .agent-data/.kilo/data` symlink). A write on one worker is visible on every other worker the same user owns.
- Kilo is a **fourth Account credential/reset row** alongside Claude/Codex/Gemini (per-user auth file `credentials/kilo.json`), but it has **no usage-monitoring endpoint** and **no separately installed Kilo CLI** — the orchestrator only manages its shared config/auth files. Do not imply Kilo usage monitoring or a standalone Kilo CLI.

### 29.3 Per-worker Kilo/code-server data (NOT shared)
- Kilo session DB/data beyond auth, plus code-server user/UI state, stay **per-worker** in `.agent-data` and survive restart/rebuild/archive — they are NOT shared across same-user workers and no shared SQLite/session state is implied.
...
- Cross-user isolation: a second user's worker cannot see the first user's Kilo config or auth markers, and each worker sees only its own owner's values.

### 29.4 Persistence across lifecycle
- Shared per-user Kilo config (bind-mounted) survives rebuild; per-worker code-server user-data (agent-data volume) survives rebuild. Both survive restart and archive/unarchive.
