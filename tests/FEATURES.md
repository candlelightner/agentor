...
- The MCP listens only on an internal management network, is preconfigured in the administrative Codex environment through a rotating-credential stdio bridge, rechecks a fail-closed live group allowlist, omits disabled groups from discovery, audits every success/failure without args/results, and supports bounded/redacted linked-tmux console sessions inside selected workers. It also exposes an owner-checked bounded/redacted Docker-log tail and sanitized read-only usage/worker-metric snapshots. Immutable configuration proposals retain optional dashboard review; the invoking harness controls confirmation while Agentor enforces identity and capabilities.
- The management MCP covers worker lifecycle/configuration/groups/locks/console, bounded running-workspace file management, offline workspace browsing, exports and a one-use private streaming worker-import handoff, image builds/catalog recovery, backups/restores, controlled networks, port/domain mappings, worker apps, storage cleanup, user-global plain variables, and user configuration catalogs. Running-file mutations reuse the hardened container file manager, require a protected worker's lock password, and cap a single base64 upload at 1 MiB. Global variables are not falsely presented as encrypted secrets: sensitive-looking names are masked on reads, while true write-only secrets remain worker-local. Its parity matrix documents non-retrieval of secrets and binary/streaming ceilings.
