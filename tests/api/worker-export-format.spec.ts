...
test.describe("Worker export root filesystem format compatibility", () => {
  test("v2 accepts an uncompressed rootfs.tar payload", async () => {
  ...
  test("legacy v1 accepts a gzip-compressed rootfs.tar.gz payload", async () => {
    const rawRootfs = await tarBuffer([{ name: "etc/issue", body: Buffer.from("legacy\n") }]);
    await withBundle([
      { name: BUNDLE_FILES.manifest, body: manifest(1) },
  ...
  test("rejects bundle versions newer than the supported format", async () => {
    await withBundle([
      { name: BUNDLE_FILES.manifest, body: manifest(WORKER_EXPORT_VERSION + 1, false) },
    ], async (bundle, destination) => {
