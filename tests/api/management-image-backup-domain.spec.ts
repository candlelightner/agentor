...
test('image and backup MCP domain exposes bounded tool surface and safe hints', () => {
  ...
  for (const name of ['images.list','images.create','images.build','images.build-logs','images.promote','images.rollback','images.git-sync','backups.list','backups.providers','backups.create','backups.cancel','backups.retry','backups.delete','backups.restore']) expect(names).toContain(name);
