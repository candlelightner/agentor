...
test.describe.serial('Managed networks dashboard', () => {
  ...
  test.beforeAll(async ({ request }) => {
    const worker = await createWorker(request, { displayName: `network-ui-worker-${Date.now()}` });
    workerId = worker.id;
    workerName = String(worker.displayName);
  });

  test.afterAll(async ({ request }) => {
    if (networkId) await request.delete(`/api/managed-networks/${networkId}`).catch(() => {});
    if (groupId) await request.delete(`/api/worker-groups/${groupId}`).catch(() => {});
    if (workerId) await cleanupWorker(request, workerId);
  });

  test('creates, validates, detaches, and deletes a group-scoped network without deleting its worker', async ({ page, request }) => {
    ...
    await expect.poll(async () => {
      const allGroups = await (await request.get('/api/worker-groups')).json() as Array<{ id: string; name: string; workerIds: string[] }>;
    ...
    const networks = page.getByTestId('managed-networks');
    ...
    const network = networks.locator('section').filter({ hasText: networkName });
    await expect(network).toContainText(`group: ${groupName}`);
    await expect.poll(async () => {
      const allNetworks = await (await request.get('/api/managed-networks')).json() as Array<{ id: string; name: string }>;
      networkId = allNetworks.find(entry => entry.name === networkName)?.id || '';
      return networkId;
    }).not.toBe('');
    await network.getByRole('button', { name: 'Validate' }).click();
    await expect(network).toContainText(/Connectivity membership verified\.|Missing:/);

    await page.keyboard.press('Escape');
    await expect(networks).toBeHidden();
    const updatedGroups = await openGroups(page);
    const updatedGroup = updatedGroups.locator('section').filter({ hasText: groupName });
    await updatedGroup.getByRole('checkbox', { name: workerName }).uncheck();
    await expect.poll(async () => (await (await request.get(`/api/worker-groups/${groupId}`)).json() as { workerIds: string[] }).workerIds).toEqual([]);
    await page.keyboard.press('Escape');
    await expect(updatedGroups).toBeHidden();

    await page.getByRole('button', { name: 'Managed networks' }).click();
    const reopenedNetwork = page.getByTestId('managed-networks').locator('section').filter({ hasText: networkName });
    await expect(reopenedNetwork).toBeVisible();
    await reopenedNetwork.getByRole('button', { name: 'Validate' }).click();
    await expect(reopenedNetwork).toContainText(/Connectivity membership verified\.|Missing:/);
    ...
    await cleanupGroups.locator('section').filter({ hasText: groupName }).getByRole('button', { name: 'Delete' }).click();
    groupId = '';
  });
});
