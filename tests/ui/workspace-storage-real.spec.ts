...
test.describe.serial("Workspace storage UI against real APIs", () => {
  test("browses, downloads, and clones a stopped workspace", async ({ page, request }) => {
    ...
    try {
      ...
      sourceId = source.id;
      expect((await api.uploadToWorkspace(sourceId, [{ name: marker, content: Buffer.from("offline UI marker") }])).status).toBe(200);
      ...
      await expect(browser.getByRole("button", { name: marker, exact: true })).toBeVisible();
      ...
      const before = (await api.listContainers()).body.map((worker: { id: string }) => worker.id);
      await row.getByRole("button", { name: "Clone workspace into a new worker" }).click();
      await expect(page.getByText("Workspace cloned", { exact: true })).toBeVisible({ timeout: 90_000 });
      const clone = (await api.listContainers()).body.find((worker: { id: string }) => !before.includes(worker.id));
      ...
      expect(Buffer.from(restored.body).toString("latin1")).toContain(marker);
    } finally {
