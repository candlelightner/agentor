...
async function seedFiles(
  ...
  const res = await request.post(`${BASE_URL}/api/containers/${containerId}/files/upload`, {
    multipart: form,
  });
  if (res.status() >= 400) {
...
async function openFilesModal(page: Page, displayName: string): Promise<Locator> {
...
test.describe.serial('Workspace Files Modal', () => {
  ...
  test('Files tooltip action opens the modal only (no other modal)', async ({ page }) => {
  ...
  test('modal closes via the header Close (X) button', async ({ page }) => {
    ...
    await page.locator('[data-testid="workspace-files-modal"] button[aria-label="Close"]').click();
    await expect(page.locator('[data-testid="workspace-files-modal"]')).toBeHidden({ timeout: 10_000 });
  });

  test('modal closes via the footer Close button', async ({ page }) => {
    await openFilesModal(page, displayName);
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Close' }).click();
    await expect(page.locator('[data-testid="workspace-files-modal"]')).toBeHidden({ timeout: 10_000 });
  });
  ...
  test('root view shows a single /workspace breadcrumb and disabled Up button', async ({ page }) => {
    await openFilesModal(page, displayName);
    ...
    await expect(bc.getByRole('button', { name: 'Up one level' })).toBeDisabled();
  });
  ...
  test('empty workspace shows the empty-folder message', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="workspace-files-list"], :text("This folder is empty.")'),
    ).toBeVisible({ timeout: 15_000 });
  });

  test('error banner with Retry appears when the listing API fails', async ({ page }) => {
    await page.route('**/api/containers/*/files?path=', async (route) => {
    ...
    await page.unroute('**/api/containers/*/files?path=');
  });
  ...
  test('seeded files and folders appear in the listing', async ({ page, request }) => {
  ...
  test('double-clicking a folder navigates into it', async ({ page }) => {
    ...
    await rowByPath(page, 'docs').locator('button').dblclick();
    // Breadcrumb now shows the docs segment.
    const bc = page.locator('[data-testid="workspace-breadcrumb"]');
    await expect(bc.getByText('docs', { exact: true })).toBeVisible({ timeout: 10_000 });
    await expect(bc.getByRole('button', { name: 'Up one level' })).toBeEnabled();
    // Contents of docs.
    await expect(rowByPath(page, 'readme.md')).toBeVisible();
    await expect(rowByPath(page, 'nested')).toBeVisible();
  });
  ...
  test('Enter on a focused folder row navigates into it', async ({ page }) => {
    ...
    await rowByPath(page, 'docs').locator('button').click();
  ...
  test('Up button returns to the parent directory', async ({ page }) => {
    ...
    await waitForList(page);
    await rowByPath(page, 'docs').locator('button').dblclick();
    await expect(page.locator('[data-testid="workspace-breadcrumb"]').getByText('docs', { exact: true })).toBeVisible();
    await page.locator('[data-testid="workspace-breadcrumb"]').getByRole('button', { name: 'Up one level' }).click();
    await expect(rowByPath(page, 'docs')).toBeVisible({ timeout: 10_000 });
    await expect(page.locator('[data-testid="workspace-breadcrumb"]').getByText('docs', { exact: true })).toHaveCount(0);
  });
  ...
  test('clicking a breadcrumb segment navigates to it', async ({ page }) => {
    ...
    await waitForList(page);
    await rowByPath(page, 'docs').locator('button').dblclick();
    await rowByPath(page, 'nested').locator('button').dblclick();
    // Breadcrumb: /workspace > docs > nested
    const bc = page.locator('[data-testid="workspace-breadcrumb"]');
    await expect(bc.getByText('nested', { exact: true })).toBeVisible({ timeout: 10_000 });
    // Click the docs crumb.
    await bc.getByText('docs', { exact: true }).click();
    await expect(rowByPath(page, 'readme.md')).toBeVisible({ timeout: 10_000 });
    await expect(rowByPath(page, 'deep.txt')).toHaveCount(0);
  });

  // ─── Multi-select / select-all ────────────────────────────────────────────

  test('select-all checkbox selects and clears all entries', async ({ page }) => {
    await openFilesModal(page, displayName);
    const list = await waitForList(page);
    const selectAll = list.locator('thead input[type="checkbox"]');
    await selectAll.check();
    await expect(page.getByText(/selected/)).toBeVisible();
    const rowCount = await list.locator('tbody tr').count();
    const checked = list.locator('tbody input[type="checkbox"]:checked');
    await expect(checked).toHaveCount(rowCount);
    await selectAll.uncheck();
    await expect(checked).toHaveCount(0);
  });

  test('individual row checkbox toggles selection and updates the count', async ({ page }) => {
    await openFilesModal(page, displayName);
    const list = await waitForList(page);
    await rowByPath(page, 'root-file.txt').locator('input[type="checkbox"]').check();
    await expect(page.getByText('1 selected')).toBeVisible();
    await rowByPath(page, 'root-file.txt').locator('input[type="checkbox"]').uncheck();
    await expect(page.getByText(/selected/)).toHaveCount(0);
  });

  // ─── Upload: exact current-folder via file picker, preserved relative paths ─

  test('upload via file picker places files in the current folder preserving relative paths', async ({ page, request }) => {
    await openFilesModal(page, displayName);
    await waitForList(page);
    ...
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Upload' }).click();
    ...
    await input.setInputFiles([
      { name: 'picker-dir/uploaded-a.txt', mimeType: 'text/plain', buffer: Buffer.from('a') },
      { name: 'picker-dir/sub/uploaded-b.txt', mimeType: 'text/plain', buffer: Buffer.from('b') },
    ]);
    ...
    await expect(rowByPath(page, 'picker-dir')).toBeVisible({ timeout: 15_000 });
    // Verify server-side the tree was preserved.
    const api = new ApiClient(request);
    const res = await request.get(`${BASE_URL}/api/containers/${containerId}/files?path=docs/picker-dir`);
  ...
  test('upload via dropzone accepts dropped files into the current folder', async ({ page }) => {
    await openFilesModal(page, displayName);
    await waitForList(page);
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Upload' }).click();
    const panel = page.locator('[data-testid="workspace-upload-panel"]');
    ...
    await expect(panel.getByText('drop-dir/dropped.txt')).toBeVisible({ timeout: 5_000 });
  });

  // ─── Upload conflict overwrite panel ──────────────────────────────────────

  test('upload conflict shows the overwrite panel and retry replaces', async ({ page, request }) => {
    ...
    const input = panel.locator('input[type="file"]').first();
    await input.setInputFiles([
      { name: 'conflict.txt', mimeType: 'text/plain', buffer: Buffer.from('replacement') },
    ]);
    await panel.getByRole('button', { name: /Upload/ }).first().click();
    ...
    await expect(panel.getByRole('button', { name: 'Overwrite & retry' })).toBeVisible();
  ...
  test('New Folder creates a directory in the current folder', async ({ page }) => {
    await openFilesModal(page, displayName);
    await waitForList(page);
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'New Folder' }).click();
  ...
  test('New Folder rejects invalid names (slashes) with a validation message', async ({ page }) => {
    await openFilesModal(page, displayName);
    await waitForList(page);
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'New Folder' }).click();
    const panel = page.locator('[data-testid="workspace-mkdir-panel"]');
  ...
  test('Rename renames a single selected entry', async ({ page }) => {
    ...
    await waitForList(page);
    await rowByPath(page, 'root-file.txt').locator('input[type="checkbox"]').check();
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Rename' }).click();
  ...
  test('Rename is disabled unless exactly one entry is selected', async ({ page }) => {
    await openFilesModal(page, displayName);
    await waitForList(page);
    const renameBtn = page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Rename' });
    await expect(renameBtn).toBeDisabled();
    await rowByPath(page, 'docs').locator('input[type="checkbox"]').check();
    await rowByPath(page, 'conflict.txt').locator('input[type="checkbox"]').check();
    await expect(renameBtn).toBeDisabled();
  });

  // ─── Move to root / nested and conflict retry ──────────────────────────────

  test('Move to root relocates selected entries to /workspace', async ({ page, request }) => {
    // Seed a nested file to move.
    await seedFiles(request, containerId, 'docs', [
    ...
    await rowByPath(page, 'docs').locator('button').dblclick();
    await expect(rowByPath(page, 'moveme-root.txt')).toBeVisible({ timeout: 10_000 });
    await rowByPath(page, 'moveme-root.txt').locator('input[type="checkbox"]').check();
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Move' }).click();
    ...
    await page.locator('[data-testid="workspace-breadcrumb"]').getByText('/workspace', { exact: true }).click();
    await expect(rowByPath(page, 'moveme-root.txt')).toBeVisible({ timeout: 10_000 });
  });

  test('Move into a nested destination relocates selected entries', async ({ page, request }) => {
    // Ensure a nested destination folder exists.
    await seedFiles(request, containerId, 'dest-nested', [
      { name: '.keep', content: '' },
    ], true);
    ...
    await openFilesModal(page, displayName);
    await waitForList(page);
    await rowByPath(page, 'moveme-nested.txt').locator('input[type="checkbox"]').check();
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Move' }).click();
    const panel = page.locator('[data-testid="workspace-move-panel"]');
    ...
    await rowByPath(page, 'dest-nested').locator('button').dblclick();
    await expect(rowByPath(page, 'moveme-nested.txt')).toBeVisible({ timeout: 10_000 });
  });

  test('Move conflict shows the overwrite panel and retry replaces', async ({ page, request }) => {
    // Source file and a conflicting target with the same basename.
    await seedFiles(request, containerId, '', [
      { name: 'move-conflict-src.txt', content: 'src' },
    ], true);
    await seedFiles(request, containerId, 'dest-conflict', [
      { name: 'move-conflict-src.txt', content: 'existing' },
    ], true);
    await openFilesModal(page, displayName);
    await waitForList(page);
    await rowByPath(page, 'move-conflict-src.txt').locator('input[type="checkbox"]').check();
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Move' }).click();
    const panel = page.locator('[data-testid="workspace-move-panel"]');
    ...
    const res = await request.get(`${BASE_URL}/api/containers/${containerId}/files?path=dest-conflict`);
    const body = await res.json();
    const names = body.entries.map((e: { name: string }) => e.name);
  ...
  test('Delete confirmation panel lists selected entries and cancels without deleting', async ({ page, request }) => {
    await seedFiles(request, containerId, '', [
      { name: 'delete-cancel.txt', content: 'x' },
    ], true);
    await openFilesModal(page, displayName);
    await waitForList(page);
    await rowByPath(page, 'delete-cancel.txt').locator('input[type="checkbox"]').check();
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Delete' }).click();
  ...
  test('Delete confirmation removes the selected entry', async ({ page, request }) => {
    await seedFiles(request, containerId, '', [
      { name: 'delete-me.txt', content: 'x' },
    ], true);
    await openFilesModal(page, displayName);
    await waitForList(page);
    await rowByPath(page, 'delete-me.txt').locator('input[type="checkbox"]').check();
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Delete' }).click();
    const panel = page.locator('[data-testid="workspace-delete-panel"]');
  ...
  test('single file download triggers a direct file download event', async ({ page, request }) => {
    await seedFiles(request, containerId, '', [
      { name: 'dl-single.txt', content: 'single-download' },
    ], true);
    await openFilesModal(page, displayName);
    await waitForList(page);
    await rowByPath(page, 'dl-single.txt').locator('input[type="checkbox"]').check();
    const downloadPromise = page.waitForEvent('download');
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Download' }).click();
    const download = await downloadPromise;
    expect(download.suggestedFilename()).toBe('dl-single.txt');
  });

  test('multi-file download triggers a ZIP download event', async ({ page, request }) => {
    await seedFiles(request, containerId, '', [
    ...
    await openFilesModal(page, displayName);
    await waitForList(page);
    await rowByPath(page, 'dl-a.txt').locator('input[type="checkbox"]').check();
    await rowByPath(page, 'dl-b.txt').locator('input[type="checkbox"]').check();
    const downloadPromise = page.waitForEvent('download');
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Download' }).click();
    const download = await downloadPromise;
  ...
  test('folder download triggers a ZIP download event', async ({ page, request }) => {
    await seedFiles(request, containerId, 'dl-folder', [
      { name: 'inside.txt', content: 'inside' },
    ], true);
    await openFilesModal(page, displayName);
    await waitForList(page);
    await rowByPath(page, 'dl-folder').locator('input[type="checkbox"]').check();
    const downloadPromise = page.waitForEvent('download');
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'Download' }).click();
    const download = await downloadPromise;
    expect(download.suggestedFilename()).toBe('workspace-download.zip');
  ...
  test('escaping symlink shows a warning icon and disables destructive actions', async ({ page }) => {
    await page.route('**/api/containers/*/files?path=', async (route) => {
      await route.fulfill({
        status: 200,
        json: {
    ...
    await expect(escapeRow.locator('.i-lucide-alert-triangle')).toBeVisible();
    // Title attribute carries the escape warning.
    await expect(escapeRow.locator('button').first()).toHaveAttribute(
      'title',
      /Symlink escapes \/workspace/,
    );
    // Select the escaping symlink: destructive actions are disabled.
    await escapeRow.locator('input[type="checkbox"]').check();
    const modal = page.locator('[data-testid="workspace-files-modal"]');
    await expect(modal.getByRole('button', { name: 'Delete' })).toBeDisabled();
    await expect(modal.getByRole('button', { name: 'Move' })).toBeDisabled();
    await expect(modal.getByRole('button', { name: 'Download' })).toBeDisabled();
    await page.unroute('**/api/containers/*/files?path=');
  });

  test('escaping symlink row is not navigable (double-click is a no-op)', async ({ page }) => {
    await page.route('**/api/containers/*/files?path=', async (route) => {
      await route.fulfill({
        status: 200,
        json: {
    ...
    await openFilesModal(page, displayName);
    await waitForList(page);
    const bcBefore = await page.locator('[data-testid="workspace-breadcrumb"]').innerText();
    await rowByPath(page, 'escape-link').locator('button').dblclick();
    // Breadcrumb unchanged (still at root).
    await expect(page.locator('[data-testid="workspace-breadcrumb"]')).toHaveText(bcBefore);
    await page.unroute('**/api/containers/*/files?path=');
  });

  // ─── Keyboard: Space / Delete / F2 / Arrow / Escape ────────────────────────

  test('Space toggles row selection via keyboard', async ({ page, request }) => {
    await seedFiles(request, containerId, '', [
  ...
  test('Delete key opens the delete confirmation panel', async ({ page, request }) => {
    await seedFiles(request, containerId, '', [
      { name: 'kb-delete.txt', content: 'x' },
    ], true);
    await openFilesModal(page, displayName);
    await waitForList(page);
    await rowByPath(page, 'kb-delete.txt').locator('input[type="checkbox"]').check();
    await rowByPath(page, 'kb-delete.txt').focus();
    await page.keyboard.press('Delete');
    await expect(page.locator('[data-testid="workspace-delete-panel"]')).toBeVisible();
  });

  test('F2 opens the rename panel for a single selection', async ({ page, request }) => {
    await seedFiles(request, containerId, '', [
      { name: 'kb-rename.txt', content: 'x' },
    ], true);
    await openFilesModal(page, displayName);
    await waitForList(page);
    await rowByPath(page, 'kb-rename.txt').locator('input[type="checkbox"]').check();
    await rowByPath(page, 'kb-rename.txt').focus();
    await page.keyboard.press('F2');
    await expect(page.locator('[data-testid="workspace-rename-panel"]')).toBeVisible();
  });

  test('ArrowDown/ArrowUp moves focus between rows', async ({ page, request }) => {
    await seedFiles(request, containerId, '', [
  ...
  test('Escape clears an open action panel before closing the modal', async ({ page }) => {
    await openFilesModal(page, displayName);
    await waitForList(page);
    await page.locator('[data-testid="workspace-files-modal"]').getByRole('button', { name: 'New Folder' }).click();
    await expect(page.locator('[data-testid="workspace-mkdir-panel"]')).toBeVisible();
    // First Escape clears the panel (modal stays open).
    await page.keyboard.press('Escape');
    await expect(page.locator('[data-testid="workspace-mkdir-panel"]')).toBeHidden({ timeout: 5_000 });
    await expect(page.locator('[data-testid="workspace-files-modal"]')).toBeVisible();
  ...
  test.describe('mobile viewport', () => {
    ...
    test('toolbar, breadcrumb, and list remain usable on a mobile viewport', async ({ page, request }) => {
      await seedFiles(request, containerId, '', [
      ...
      await expect(modal.getByRole('button', { name: 'New Folder' })).toBeVisible();
      // Breadcrumb is horizontally scrollable (overflow-x auto).
      const bc = page.locator('[data-testid="workspace-breadcrumb"]');
      const overflowX = await bc.evaluate((el) => getComputedStyle(el).overflowX);
      expect(['auto', 'scroll']).toContain(overflowX);
      // The list row is visible and selectable.
      await expect(rowByPath(page, 'mobile.txt')).toBeVisible();
      await rowByPath(page, 'mobile.txt').locator('input[type="checkbox"]').check();
      await expect(page.getByText('1 selected')).toBeVisible();
    });
  });
  ...
  test('stopped worker shows a recoverable error and refresh works after restart', async ({ page, request }) => {
    const api = new ApiClient(request);
    await api.stopContainer(containerId);
    try {
      await openFilesModal(page, displayName);
      // The card may still show "running" briefly; the modal surfaces the
      // stopped error once the listing request fails or the status watch fires.
      const err = page.locator('[data-testid="workspace-files-error"]');
      await expect(err).toBeVisible({ timeout: 30_000 });
      await expect(err.getByText(/not running/)).toBeVisible();
      // Refresh button is present (recoverable).
      await expect(err.getByRole('button', { name: 'Retry' })).toBeVisible();
    } finally {
