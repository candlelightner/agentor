...
- **Worker-self API isolation** — every ordinary-worker request under `/api/worker-self/*` is source-IP identified and then checked against the current durable per-worker/per-group `inherit | allow | deny` policy. Worker override → nearest group ancestor → backward-compatible allow; invalid/overlapping denied hierarchies fail closed. GUI, REST, and management MCP share the same effective state. Changes apply to the next request without a worker restart/rebuild; trusted platform/group admin workspaces retain only their scoped management/plugin MCP exception.
...
Whole-instance migration uses the platform-admin-only `AGENTOR-INSTANCE-BACKUP-1` disaster-recovery path: a SQLite online snapshot of `auth.db`, versioned `DATA_DIR` state (including image/template/plugin definitions and desired installations), optional Agentor Docker volumes, and distinct local/Google Drive discovery/adoption. Restore is limited to an empty `AGENTOR_INSTANCE_RECOVERY_MODE=true` installation and a network-disabled rollback helper. See @docs/instance-disaster-recovery.md.

### Worker runtime recovery

Docker inventory alone is not proof that a worker task is healthy. Agentor reports an unverifiable runtime as `unknown` with a retryable, redacted diagnostic; it does not present stale `running` health. Use the worker **Recover** action or `POST /api/containers/:id/recover` for that worker only. Recovery verifies persistent mounts first, then replaces only disposable compute and re-runs secret/plugin bootstrap; it preserves the worker record, workspace, agent data, DinD, and selected persistent-path volumes. If persistence cannot be verified, recovery must leave Docker untouched. Desired running/stopped state is durable across daemon restart: secret-bearing workers wait for Agentor bootstrap, while explicitly stopped workers stay stopped.
...
| Instance disaster recovery | @docs/instance-disaster-recovery.md | Encrypted control-plane snapshots, Google Drive discovery/adoption, recovery-mode restore, rollback and external dependencies |
...
**Requirements**:
- **End-to-end testing is mandatory before declaring any feature done.** A feature is NOT complete because an API endpoint, UI modal, or composable exists. "Done" means a real user can reach and exercise the full path end-to-end (UI → API → persistence → worker side effects, and back) and that path is covered by passing automated tests. Before claiming completion, run the feature's API **and** UI Playwright specs against the Dockerized stack and confirm they pass; for flows that touch a worker, exercise a real worker round-trip (create → use → restart/rebuild/archive/restore as relevant). Do not rely on typecheck or unit-level assertions alone.
- **This applies retroactively to the entire platform-expansion sprint** (the eight workstreams: export/import, custom image builder & catalog, backup & restore, offline volume inventory & browsing, worker-scoped variables & secrets, admin workspace, internal management MCP, git-backed image catalog). Any of these not yet verified by an end-to-end Playwright run (API + UI, real worker where applicable) must be treated as still in progress and verified before being called complete. See `NEEDED_FIXES.md` for known gaps.
