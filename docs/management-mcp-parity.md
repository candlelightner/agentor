...
| Backups, restore, provider status/settings | Backup UI/API | Platform principal: `backups.list/providers/settings/create/status/cancel/retry/delete/restore`; `backups.restore` accepts an optional exact `workspaceIds` subset and otherwise restores every artifact workspace. Group principal: `backups.create` and `backups.paths.list` accept the same explicit absolute file/directory selections as the GUI, with owner derived from the bound identity and scope limited to the subtree plus the calling group-admin workspace; backup job status/cancellation are similarly scoped. `backups.restore` remains undiscovered because a restore creates workers outside its fixed group assignment. | Backup, Google OAuth installation, group-admin MCP, and MCP image/backup tests. MCP restore creates new workers only and therefore does not mutate the source. Provider credentials/tokens are intentionally non-retrievable; Google login remains human/external boundary |
