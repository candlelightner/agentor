...
| Managed local persistent paths | Worker storage controls and `/api/volumes*`, `/api/containers/:id/storage` | `volumes.inventory/inspect/rename/delete`, `workers.storage.{inspect,add,apply,detach,reattach}` | `managed-volumes`, `managed-volume-store`, `managed-volume-helper`, and `managed-volume-mcp` coverage. The service accepts only approved additive paths; live mount/recreation and deletion remain explicit safety boundaries. Detached records survive deleted-account cleanup for administrator action. On-demand sizing and portable custom-volume backup/export/restore are not implemented. |
