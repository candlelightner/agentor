...
| Worker groups | Worker Groups UI; `/api/worker-groups`; group workspace lifecycle at `/api/worker-groups/:id/admin-workspace` (`GET`, `POST`, and `/start`, `/stop`, `/rebuild` `POST`s) | Platform principal: `groups.list`, `groups.create`, `groups.update`, `groups.delete`, plus `groups.admin-workspace.get`, `.provision`, `.start`, `.stop`, and `.rebuild`; group principal: group inspection only, with no group/membership mutation | `worker-groups.spec.ts` and group-workspace API/UI E2E exercise real Docker topology and scope: membership updates reconcile dependent networks, a platform principal can provision and control the resulting group workspace, a group workspace sees and acts only on current members, known out-of-group IDs fail, and it cannot expand membership or mutate group administration |
