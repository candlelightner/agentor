...
console/logs, files/workspaces (excluding clone), exports, backup creation and
status (not artifact restore), apps, and
...
| Worker create, settings, restart, recover, rebuild, archive, unarchive, delete | Worker dialogs; container lifecycle API, including `POST /api/containers/:id/recover` | `workers.create`, `workers.update`, `workers.restart`, `workers.recover`, `workers.rebuild`, `workers.archive`, `workers.unarchive`, `workers.delete` | Recovery verifies persistent mounts and replaces only disposable compute before managed secret bootstrap/plugin reconciliation; 409/503 diagnostics are structured, retryable, and value-free. Durable desired running/stopped intent is distinct from observed `starting`/`recovering`/`unknown`; lock password applies where required. |
...
| Recursive worker groups and administrative startup scripts | Nested Worker Groups UI; `/api/worker-groups`, `/assignment`, and `/validation`; group workspace lifecycle and `/startup-script` at `/api/worker-groups/:id/admin-workspace`; platform workspace `/api/admin/workspace/startup-script` | Platform principal: `groups.list/create/update/assign-worker/delete`, admin-workspace lifecycle, identity-bound `admin-workspace.startup-script.get/set`, and any provisioned group via `groups.admin-workspace.startup-script.get/set`; group principal: self script without a target ID plus lifecycle/script management for authorized descendants. `groups.list` mirrors REST `memberCounts` for direct total/active/archived membership. | Hierarchy/API/UI tests cover legacy roots, arbitrary depth, cycle/cross-owner rejection, overlap diagnostics, atomic moves, recursive network reconciliation/rollback, retained archived membership and counts, live ancestor authority, sibling/parent denial, descendant enrollment, non-disruptive desired/applied script revisions, every-start execution, failure status, and volume persistence |
| Governed host bind mounts | Host mount permissions dialog; `/api/host-mounts*`; catalog dropdown + read-only/read-write selection in worker create/settings; optional group selection at create | Platform principal: `host-mounts.catalog.list/create/update/delete`, `host-mounts.entitlements.list/set`, `host-mounts.grants.list/create/delete`; `workers.create` accepts `workerGroupId`; worker mount schemas accept `pathId`, `target`, `readOnly` only. Group principal: `host-mounts.delegations.list/create/delete` only; list output separates worker-usable from explicitly delegable identities and omits raw host sources. | Empty/read-only defaults, protected host paths, entitlement and direct-group resolution, no raw MCP source, downward-only delegation, same-owner enforcement, group-create enrollment, active revocation stop/restart guard/rebuild removal, REST/MCP schema tests. Group identity supplies the authority group and cannot override it. See `docs/host-mounts.md`. |
| Governed hardware devices | Hardware permissions dialog; `/api/hardware-devices*`; entitled-device selection in worker create/settings | Platform principal: `hardware-devices.discover`, `hardware-devices.catalog.list/approve/update/delete`, `hardware-devices.entitlements.list/set`, and `hardware-devices.grants.list/create/delete`; worker create/update accepts opaque `hardwareDeviceIds`. Group principal: `hardware-devices.delegations.list/create/delete` only. | Platform approval from live discovery, stable DRM/USB identity, account entitlement, downward-only group delegation, live node resolution, Docker device/group mapping, active revocation stop/restart guard/rebuild removal, and no raw device path in REST or MCP. See `docs/hardware-devices.md`. |
...
| Managed local persistent paths | Worker storage controls and `/api/volumes*`, `/api/volume-size-jobs*`, `/api/containers/:id/storage` | `volumes.inventory/inspect/rename/delete`, `volumes.size.{start,inspect,cancel}`, `workers.storage.{inspect,add,apply,detach,reattach}` | `managed-volumes`, `managed-volume-store`, `managed-volume-helper`, `managed-volume-sizing`, `managed-volume-sizing-control`, `managed-volume-sizing-mcp-authority`, and `managed-volume-mcp` coverage. The service accepts only approved additive paths; live mount/recreation and deletion remain explicit safety boundaries. Detached records survive deleted-account cleanup for platform-administrator action. Size scans are explicit, cancellable, bounded, incarnation checked, and live-authorized after initialization/state-queue waits; group controls recheck current credential, policy, workspace binding, durable ownership, and descendant scope. Job control never treats physical discovery as authority. Inventory reads only expose cached known/stale/unknown allocated/logical values. Portable capture is a separate default-false option on export and backup tools; it never expands storage-management authority. |
...
| Worker export/import | Export modal/jobs; export/import APIs | `exports.create`, `exports.status`, `exports.cancel`, `exports.download`, `imports.prepare` | `exports.create.includeManagedVolumes` is a strict default-false boolean matching REST and GUI; it captures eligible attached custom volumes only, excludes detached volumes, and marks running capture best-effort. Export-job, download-handoff, workspace-adapter, and import-domain coverage applies. `exports.download` and `imports.prepare` return short-lived, one-use, workspace-bound private handoffs using the current management credential. Neither direction base64-buffers an archive in MCP JSON. |
| Image catalog and controlled builds | Image Catalog UI/API | `images.list/get/create/validate/delete/delete-version/usage/build/build-status/build-logs/build-cancel/validation-retry/promote/rollback/default/cleanup/git-status/git-sync` | `images.build` accepts promptly, then its returned next actions lead to `images.build-status` → bounded incremental `images.build-logs` → `images.build-cancel`; validation retry and test-worker creation use the same durable job surface. `requestId` makes an identical accepted request idempotent and rejects a different request using the same id. Safe-mode rejection is actionable and pre-Docker; Advanced remains inside the controlled build boundary. Logs and Git sync responses are sanitized. |
| Image test worker / image selection | Image Catalog test-worker/default/worker creation controls | `images.test-worker`; catalog-aware `workers.create` | Both paths resolve the same immutable digest/runtime image through the shared catalog model. Promotion and test-worker creation require compatibility `ready` or `ready-with-warnings`; `built-incompatible` is not promotable and `validation-unavailable` is retryable. |
| Backups, restore, provider status/settings | Backup UI/API | Platform principal: `backups.list/providers/settings/create/status/cancel/retry/delete/restore`; settings and create accept the same strict default-false `includeManagedVolumes` as REST/GUI. `backups.restore` accepts an optional exact `workspaceIds` subset and otherwise restores every artifact workspace. Group principal: `backups.create` and `backups.paths.list` accept the same explicit absolute file/directory selections as the GUI, with owner derived from the bound identity and scope limited to the subtree plus the calling group-admin workspace; backup job status/cancellation are similarly scoped. `backups.restore` remains undiscovered because a restore creates workers outside its fixed group assignment. | Backup, Google OAuth installation, group-admin MCP, and MCP image/backup tests. MCP restore creates new workers only and therefore does not mutate the source. Custom-volume restore uses fresh identities and grants no self-service/live/recreation policy. Provider credentials/tokens are intentionally non-retrievable; Google login remains human/external boundary |
| Whole-instance disaster recovery | Platform-admin Instance disaster recovery UI; `/api/admin/instance-backups/**` | `instance-backups.list/create`, `instance-backups.discovery.start/list`, `instance-backups.inspect/adopt/preflight/restore`, and `instance-backups.status/logs/cancel` | Platform principal only; MCP `ownerId` must identify a current platform administrator so the instance artifact cannot be assigned to an ordinary user's recovery-key/provider namespace. Local and Google Drive snapshots use a separate authenticated format/provider marker from portable worker backups. Create/discovery/adoption/verification/restore are durable async operations whose start result names the exact status/log/cancel tools and arguments. Stable `requestId` retries are idempotent; REST additionally accepts the same standard `Idempotency-Key` header and the UI reuses it after uncertain responses. MCP exposes fingerprints and key availability but never raw recovery material; reveal/export remains a fresh-reauthenticated human boundary. Applying a restore requires an empty destination, matching storage mode/prefix, explicit destructive/external-dependency acknowledgements, an acceptance-to-handoff mutation barrier, a second post-stop emptiness check, and hardened helper restart/rollback. See `docs/instance-disaster-recovery.md`. |
