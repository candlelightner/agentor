...
Google backup linking uses a separate OAuth client and is unrelated to Agentor login. Configure `GOOGLE_BACKUP_CLIENT_ID`, `GOOGLE_BACKUP_CLIENT_SECRET`, and the exact registered `GOOGLE_BACKUP_REDIRECT_URI` ending in `/api/backup-providers/google/oauth/callback`; optionally set `GOOGLE_BACKUP_FOLDER_ID`. OAuth state is one-time and expires after ten minutes. Tokens are AES-GCM encrypted at rest and refreshed by the provider. Tests use the explicitly gated fake provider and never require a real Google account.
...
The orchestrator generates and pins the trusted administrative overlay (`AGENTOR_ADMIN_WORKER_IMAGE` controls only its server-side name). It joins the normal worker network solely for existing terminal/editor/desktop proxying and the internal `agentor-management` network for MCP. The MCP address has no published port or Traefik route. Ordinary workers never join the management network. The admin container has no Docker socket, host bind, privilege, or public port; its short-lived MCP identity lives only in tmpfs.

Mutating MCP groups default off. Every invocation rechecks current policy and creates a sanitized audit record. Configuration application requires an immutable proposal and an approval performed through the trusted dashboard; the workload identity cannot approve. Worker log bodies are intentionally omitted from MCP responses because generic log text cannot be proven free of bare secrets.
