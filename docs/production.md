...
## Worker runtime convergence

Docker observations are bounded: normal reads use short deadlines and lifecycle mutations use bounded deadlines, with fixed value-free operation labels. A disconnected HTTP client cancels request-scoped preparation safely; late Docker completion remains observed rather than becoming an unhandled rejection. The UI/API can therefore report `unknown` or `recovering` rather than claiming a timed-out task is stopped. `desiredRuntimeStatus` persists the requested `running`/`stopped` intent across Docker and orchestrator restarts; startup reconciliation preserves a deliberate stop and retries intended running workers one at a time.

Secret-bearing workers are intentionally `restart=no`: only Agentor may start them, so their tmpfs secret material can be restored through the bounded bootstrap retry path. Their task probe also verifies the root-owned bootstrap marker; a legacy worker started directly by Docker or an operator with an empty tmpfs is therefore `unknown` and is restarted through Agentor rather than accepted as healthy. If a worker cannot be queried, stopped, or restarted normally, use the owner/lock-checked `POST /api/containers/:id/recover` or management MCP `workers.recover`. Recovery verifies every persistent mount, removes/recreates only disposable compute, then reapplies managed bootstrap and desired plugins. It preserves workspace, agent data, DinD, selected persistent paths, worker/group identity, mappings, and plugin desired state. A 409/503 structured diagnostic is retryable; do not delete volumes to work around an unavailable daemon. Older records without a durable intent are migrated only after a verified runtime observation, so ambiguous legacy Docker state is reported as unknown instead of being guessed.
...
- **disk** is sampled on the slower 60s cadence using a bounded in-worker `du` of `/workspace` plus persistent agent data. It intentionally avoids Docker's expensive `inspect({ size: true })` storage-driver walk, which can make a wedged task interfere with lifecycle recovery. The metric therefore describes durable worker state, not disposable root-filesystem changes or the DinD image store. It is kept in a separate map and overlaid onto each `WorkerMetrics` on read. Manual refresh bypasses both interval guards so a just-created worker is reliably sampled.
...
- `managed-volumes.tar.gz` — only in opted-in v6 bundles, with a strict manifest for eligible attached custom volumes. Detached volumes are excluded, and running capture is best-effort. Inner archives preserve ordinary files, directories, safe relative symlinks, target-confined absolute symlinks, backward hardlinks to earlier regular files, and long paths/links represented by bounded per-file PAX `path`/`linkpath` metadata. Global PAX records, GNU long-name/link records, unknown or authority-bearing PAX keys and xattrs, special files, path/link escapes, and normalized path/type collisions fail closed.
...
**Import** (`POST /api/containers/import`, raw `.tar` body streamed to disk, `ContainerManager.importWorker`): admits one import per user, rejects oversized bodies while streaming, preflights temporary space, strictly validates the outer manifest/tar, and scans compressed inner volume tars plus current/legacy compressed or transitional v2 uncompressed rootfs tar for expanded-size, entry-count, and traversal limits before any Docker mutation. V1–v5 bundles retain their legacy meaning; only v6 may instruct portable custom-volume restore. It then mints a fresh UUID worker and fresh labelled destination volumes, preserves approved target paths without trusting imported Docker identities or host paths, and leaves persistence self-service, live-mount, and recreation policies false. Captured rootfs import is bounded to 40 minutes and fails closed rather than silently substituting the standard image; recover/rebuild the referenced definition, select an explicit replacement, or explicitly choose workspace-only restore. Rebuild/recovery also preflight the required image before replacing disposable compute. The per-worker image link (`importedImage` on the `WorkerRecord`) survives rebuild/unarchive and is removed on permanent delete.
...
For Git recovery, optionally set `GIT_IMAGE_CATALOG_ENCRYPTION_KEY` or preserve the generated 0600 key file. GitHub App mode also requires a server-mounted PEM at `GITHUB_APP_PRIVATE_KEY_FILE`; the PEM never enters persistent workspace storage. Fine-grained PATs should be scoped to one selected repository with **Metadata: read** and **Contents: read/write**; pull-request workflow additionally needs **Pull requests: read/write**, and GitHub-Actions build mode needs **Actions: read/write** to dispatch the already existing workflow. The catalog form exposes the effective requirements beside the PAT field. Public no-token repositories, direct/branch/pull-request sync, existing-workflow Actions dispatch, optional GHCR digest metadata, and disconnect/credential erasure are supported. The first `Sync local changes` initializes a completely empty GitHub repository through GitHub's required Contents-API bootstrap and then writes the catalog automatically; no manual seed commit is required. `Recover / pull` requires a catalog previously written by Agentor. Catalog recovery restores image metadata and digest-pinned GHCR references only—it is not a workspace backup.
