...
The **Storage** API exposes managed local persistent paths through `GET /api/volumes`, `GET`/`POST /api/volumes/:id`, and `GET`/`POST /api/containers/:id/storage`. Ordinary users see only their own resources; a current platform administrator's volume inventory is installation-wide and includes retained/orphan classifications, while group-management MCP inventory is limited to the group's live same-owner descendant subtree. It accepts only canonical, non-overlapping application-data directories and never exposes Docker volume names or host paths. Live mounting and recreation are separately authorized, and detach retains data until a separate confirmed deletion. Account deletion retains detached volume records for platform-administrator action rather than silently removing data. `POST /api/volumes/:id/size-jobs` starts an explicit bounded read-only scan; `GET`/`DELETE /api/volume-size-jobs/:jobId` inspect or cancel it. Job controls recheck current user/role or MCP credential/policy/workspace scope and durable ownership after asynchronous waits; they do not use physical Docker discovery as authority, so an authorized job remains cancellable if its source volume is missing. Inventory GETs only read the 15-minute incarnation-bound cache and report known/stale/unknown allocated and logical sizes. Worker exports and backups accept a strict boolean `includeManagedVolumes` option, default false. Opt-in captures eligible attached custom volumes only; detached volumes are reported as excluded, and running-worker capture is explicitly best-effort.
...
The **Backups** API uses `/api/backups`, `/api/backup-jobs`, `/api/backup-settings`, and `/api/backup-providers`. It supports owner-scoped manual/scheduled multi-workspace jobs, cancel/retry/status, restore, retention deletion, provider status, independently linked Google OAuth, and gated fake-provider diagnostics. `includeManagedVolumes` is an optional strict boolean on settings and manual backup requests; omission and legacy stored records normalize to false. When enabled, each member may carry eligible attached custom volumes, with detached volumes excluded and running capture marked best-effort. A running worker's readable directories can be browsed as metadata with `GET /api/containers/:id/backup-paths`; any additional absolute paths selected for a backup are explicit portable copies rather than an expansion of normal workspace-file access. The default backup payload is `/workspace` plus credential-filtered per-worker agent data; shared user Kilo config/data is excluded unless explicitly selected. `POST /api/backups/:id/restore` accepts an optional non-empty, duplicate-free `workspaceIds` subset of the artifact's workspaces; omission restores every workspace in the artifact, including artifacts created before selective restore was introduced. New-worker restores create only that selected subset and restore its captured agent-data. Restored custom volumes receive fresh local identities at their original targets; imported names never grant authority, and persistence self-service, live mount, and recreation policies remain false. An original-worker restore accepts exactly one selected artifact workspace, replaces its workspace only, and requires that selected worker to be stopped, plus explicit overwrite confirmation and a transient write-only `lockPassword` when that worker is protected; it rejects artifacts with explicit extra paths. Restoring into a new worker does not mutate a source and needs no lock credential. Invalid subsets are rejected before a restore job is created. Administrators configure installation Google OAuth material through `/api/admin/backup-providers/google-oauth`; the client secret is write-only and encrypted at rest. Responses expose safe progress/errors and encrypted-token status, never lock passwords, token values, client secrets, archive keys, Docker volume names, or host paths.
