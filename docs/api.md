...
The **Storage** API also exposes owner-scoped managed local persistent paths: `GET /api/volumes`, `GET`/`POST /api/volumes/:id`, and `GET`/`POST /api/containers/:id/storage`. It accepts only canonical, non-overlapping application-data directories; it never exposes Docker volume names or host paths. Live mounting and recreation are separately authorized, and detach retains data until a separate confirmed deletion. Account deletion retains detached volume records for an administrator rather than silently removing data. Capacity is Docker-managed: on-demand sizing and a portable custom-volume backup/export-and-restore workflow are planned, not implemented.
