...
**Tag groups (17):** Containers (incl. worker export/import + per-worker metrics, **workspace file manager** `/files/*` with `FileEntry`/`FileListing`/mutation schemas, and **worker clipboard**), Tmux, Apps, Port Mappings, Domain Mappings, Environments, Capabilities, Instructions, Archived Workers, Logs, Updates, GitHub, Usage, Metrics (per-worker resource metrics, via the Docker API — no host metrics), Config, Health, Worker Self — plus an "Internal" tag for proxy/WebSocket relay routes.
