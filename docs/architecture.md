...
A worker can be **exported** to a single portable v3 `.tar` bundle through durable owner-scoped export jobs (`POST /api/containers/:id/export-jobs`, status/cancel/download under `/api/export-jobs/:jobId`). Jobs persist under `users/<userId>/export-jobs.json`; completed artifacts are private streamed files with automatic expiry. Workspace-only is the default, while `rootfs.tar.gz` is an explicit advanced option. Parallel level-1 gzip avoids the historical single-core compression timeout while keeping staging and artifacts smaller than the transitional raw format. The archive pipeline is abortable, queue-bounded, disk-preflighted, and reports phases/bytes. Bundles contain `manifest.json`, `workspace.tar.gz`, `agents.tar.gz`, and optional rootfs; account credentials, Kilo shared data/config, environment variable values, and domain basic-auth credentials are stripped. Import accepts current v3 and legacy v1 gzip rootfs bundles plus transitional v2 raw `rootfs.tar` bundles. **Import** (`ContainerManager.importWorker`) restores the bundle as a brand-new worker (fresh UUID): it resolves/recreates the non-secret environment definition, `docker import`s the rootfs into a per-worker image (replicating the standard image's entrypoint/env), creates the container stopped, restores the two volumes via `putArchive`, starts it, and recreates non-secret mappings. Captured-rootfs failure is explicit and retryable; it never silently substitutes the standard image. See @docs/production.md for the full flow.

### Backups and restores

`BackupManager` persists owner-scoped configuration, jobs, and artifact metadata beneath each user's data directory. A two-slot queue exports one or multiple workspaces, packs a versioned outer bundle, encrypts it with AES-256-GCM, uploads through the provider interface, downloads it again, and verifies both the recorded hash and authenticated encryption before reporting success. Each worker member contains its `/workspace`, a credential-filtered `/home/agent/.agent-data`, and a manifest with non-secret worker/environment metadata plus port/domain mappings and missing-secret names. Backups deliberately exclude the container root filesystem/image, DinD data, environment/secret values, OAuth credentials, and shared per-user Kilo config/data. Running workers are explicitly best-effort; stopped workers use a consistent volume view; archived workspaces are read through a capability-free, networkless, read-only helper rather than starting the original worker.
