...
Workers may also use **managed local persistent paths**: an approved absolute application-data directory is backed by an Agentor-managed Docker volume and redeclared on restart/rebuild. The target cannot overlap system, runtime, credential, workspace, agent-data, DinD, or another persistent path. Adds are owner scoped and idempotent; live application and recreation require separate authorization, and a busy live path fails safely rather than being replaced. Detach retains the data, while deletion is a distinct confirmed operation. Named-volume sizes are calculated only by explicit asynchronous requests—never during inventory reads—using a 60-second/1,000,000-entry read-only helper and a 15-minute incarnation-bound cache. Results report allocated blocks separately from hardlink-deduplicated logical file bytes; live mounts are explicitly approximate. Portable export and backup capture is a separate default-off path for eligible attached custom volumes. Import validates targets before mutation, creates fresh destination identities, and leaves self-service, live-mount, and recreation policies disabled.
...
Worker lifecycle also persists `desiredRuntimeStatus` independently of Docker's observation. Docker task probes are bounded and may yield `unknown` with a value-free retryable diagnostic; normal state therefore includes `starting` and `recovering` rather than overloading `running`. Startup reconciliation serializes with lifecycle mutations, preserves deliberate stops, converges desired-running workers, and does not archive a provisional worker while its create/rebuild/recovery mutation is active. Secret-bearing workers are migrated to `restart=no` so Docker cannot restart them without Agentor repopulating tmpfs material; managed bootstrap retries are bounded. Explicit managed recovery verifies every persistent mount, force-replaces only the container, then restores bootstrap and desired plugin state. It preserves all persistent volumes and reports structured retryable diagnostics if Docker cannot establish that boundary. Legacy records gain intent only from a verified observation; ambiguous runtime remains unknown.

Workers may additionally be protected with a separate password lock. Agentor stores only a salted scrypt verifier in `worker-protection-locks.v1.json` (0600); neither the password nor verifier is returned by APIs, MCP, UI state, logs, exports, or backups. Protected workers require a transient write-only password for every material lifecycle/configuration path, including start/stop/restart, settings, local configuration, running-workspace file mutations, apps, exposure or managed-network membership, archive/unarchive, rebuild, in-place backup restore, and permanent deletion. Alternate REST and MCP aliases share the same verifier; deletion by mapping id/port resolves the affected worker before mutation. Restoring a backup into a separate new worker is allowed because it does not mutate the protected source. There is no password-recovery bypass: the owner or an Agentor administrator must supply the current password to change or remove a lock.

The protection lock guards Agentor control-plane mutation paths; it is not an authorization layer inside the worker. A user or administrative agent already authorized to use a worker terminal/console can still run commands there, including commands that change that worker's workspace. Console sessions retain their existing ownership and management-MCP capability checks. This explicit boundary preserves ordinary terminal access and avoids presenting command interception as a security control.
...
| Remove user data (on user delete) | `rm -rf /data/users/<userId>/`; custom-volume records are first retained outside that owner partition | same; retained custom-volume records remain under `/data/retained-storage/` |

When a user is deleted from the auth DB, the orphan sweeper (`server/utils/orphan-sweeper.ts`) first drains owner activity and removes every live/archived worker, then prunes each user-scoped store's in-memory cache and recursively removes `/data/users/<userId>/` — taking workspaces, agent dirs, credentials, and every per-user JSON file with it. Before that partition is removed, custom-volume records are copied into the administrator-retained store as detached records, so their Docker data is not silently deleted with the account. Worker mutations and cleanup share an owner→worker lifecycle fence and revalidate the auth owner after acquiring it, so queued create/settings/rebuild/unarchive work cannot recreate deleted state. Docker/storage/filesystem cleanup failures retain durable worker or owner handles and defer the remaining deletion; scanning the physical user directories ensures even a partially pruned owner remains discoverable after restart. It runs at startup and every 10 minutes.
...
A worker can be **exported** to a single portable bundle through durable owner-scoped export jobs (`POST /api/containers/:id/export-jobs`, status/cancel/download under `/api/export-jobs/:jobId`). Jobs persist under `users/<userId>/export-jobs.json`; completed artifacts are private streamed files with automatic expiry. Workspace-only is the default, while `rootfs.tar.gz` and attached custom-volume capture are independent explicit options. Default exports remain v5. Opting into `includeManagedVolumes` emits v6 with a strict manifest and bounded `managed-volumes.tar.gz`; even an eligible-empty capture is explicit. Detached custom volumes are excluded, and a running capture is best-effort. The archive pipeline is abortable, queue-bounded, disk-preflighted, and reports phases/bytes. Account credentials, Kilo shared data/config, environment variable values, domain basic-auth credentials, Docker volume names, and host paths are stripped. Import retains v1–v5 compatibility and validates v6 manifest/member agreement and destination target conflicts before mutation. **Import** (`ContainerManager.importWorker`) restores the bundle as a brand-new worker (fresh UUID), creates fresh labelled destination volumes at unchanged targets, and keeps all imported persistence policy flags false. Captured-rootfs failure is explicit and retryable; it never silently substitutes the standard image. See @docs/production.md for the full flow.

### Backups and restores

`BackupManager` persists owner-scoped configuration, jobs, and artifact metadata beneath each user's data directory. A two-slot queue exports one or multiple workspaces, packs a versioned outer bundle, encrypts it with AES-256-GCM, uploads through the provider interface, downloads it again, and verifies both the recorded hash and authenticated encryption before reporting success. Each worker member contains its `/workspace`, a credential-filtered `/home/agent/.agent-data`, and a manifest with non-secret worker/environment metadata plus port/domain mappings and missing-secret names. Backups deliberately exclude the container root filesystem/image, DinD data, environment/secret values, OAuth credentials, and shared per-user Kilo config/data. The strict default-false `includeManagedVolumes` setting/request option adds eligible attached custom volumes; detached volumes remain excluded. Running workers are explicitly best-effort; stopped workers use a consistent volume view; archived workspaces are read through a capability-free, networkless, read-only helper rather than starting the original worker.

Restore to new workers is rollback-clean across a multi-workspace bundle. Callers may select any non-empty exact subset of artifact workspace IDs; omitting the selection restores every member, and the same selection works for artifacts created before the UI exposed it. Original-worker restore accepts exactly one selected member and requires that worker to be stopped, explicit overwrite confirmation, and the selected worker's write-only lock password when protected; verification occurs before a durable restore job is created, then a same-filesystem staged swap provides rollback. The commit is serialized with every lifecycle mutation for that worker, preventing a concurrent start, rebuild, archive, or deletion from racing the stopped-worker check. Active restores pin their source artifact so explicit deletion and retention cannot race an in-progress download. Account cleanup first fences, aborts, and drains both asynchronous and legacy restores before deleting provider artifacts or any other owner state; a bounded drain deadline defers the entire orphan cleanup to a later sweep if a Docker mutation cannot settle promptly.
...
The additive image catalog stores owner-scoped definitions and immutable versions; older definitions are normalized when read, so missing provisioning fields become **Safe** mode. This is data normalization, not a separate database migration. Controlled builds accept only approved Agentor base aliases, bounded canonical context files, and server-rendered provisioning; legacy Dockerfile fragments remain compatible only in Safe mode when structured provisioning is absent. Advanced requires structured commands/scripts and an empty fragment. The orchestrator generates the final Dockerfile, forces the final `agent` user, applies build limits, records the immutable Docker image ID, and never exposes the Docker socket or account credentials to workers or build inputs. Fake builds are explicitly gated for tests.

Safe is the default and rejects disallowed provisioning with an actionable diagnostic before Docker is attempted: it identifies the blocked field/step, constraint, remediation, and that choosing Advanced requires an explicit review. **Advanced** permits arbitrary build-time shell only within Agentor's generated, controlled Docker-build boundary. It can produce an unusable derived image, but grants neither host access, a Docker socket, raw Dockerfile execution, nor unrestricted base-image selection; both modes reject secrets.

Builds, compatibility-validation retries, and smoke-test worker creation are durable asynchronous image jobs. A caller starts work and then polls the same job through status, reads bounded incremental sanitized logs, or requests cancellation. A caller-provided `requestId` returns the originally accepted job for the same operation and request fingerprint, while a reuse for different work is rejected. A completed controlled build is not automatically usable: validation records distinct `ready`, `ready-with-warnings`, `built-incompatible`, or `validation-unavailable` outcomes. Only ready/warning versions can be promoted or used for a test worker; incompatible versions need correction and unavailable validation can be retried.
...
The trusted `agentor-admin-worker` overlay is generated only by the orchestrator from the official worker base and then pinned by immutable image ID. It has persistent workspace/agent-data volumes, the normal terminal/editor/desktop services, red identity markers, and an orchestrator-generated non-secret control representation. It receives no Docker socket, host bind, published port, privilege, or user-selected image. Its primary network is the internal `agentor-management` network for MCP. A second dedicated `agentor-admin-egress-v1` bridge contains only the administrative workspace and provides outbound package/model API access without joining the ordinary worker network. The administrator who rebuilds the singleton workspace becomes its environment owner; that account's standard worker environment variables are injected on recreation.

Platform and group administrative workspace records also persist an optional non-secret startup script with monotonic desired/applied revisions. Docker environment configuration is immutable, so saving never destroys a running workspace: it reports `pendingRebuild`, and the next explicit start or rebuild recreates only disposable compute with the new revision. The script then runs through the worker's memfd init mechanism in the main tmux pane after every container start. A revision-bound status file under persistent agent data reports only running/succeeded/failed timing and exit metadata; script content is never copied into status or audit logs. Foreground services may remain running, while script failure returns control to an interactive shell and never blocks the bounded container readiness check. Workspace and agent-data volumes are preserved throughout.

The MCP transport listens only on the orchestrator's management-network address. Authentication uses a workspace-bound, short-lived credential held in admin tmpfs and refreshed atomically by the orchestrator. The administrative worker preconfigures Codex with a local stdio bridge; the bridge reads the current credential for every request and forwards only to the internal Docker DNS endpoint. Every call rechecks the current explicit allowlist, disabled groups disappear from discovery, and a sanitized success/failure audit event is recorded. Large offline workspace file/directory downloads, completed export artifacts, and worker imports use short-lived one-use paths on that same private listener. Download tokens capture exact normalized paths or an exact job plus its owner, recheck ownership and live capability on redemption, require the current workspace credential, and stream with backpressure and disconnect cleanup; no session cookie, public route, credential-bearing URL, or base64 archive is involved. Configuration proposals remain immutable and may be reviewed in the dashboard, but dashboard approval is optional: confirmation policy belongs to the invoking harness while Agentor enforces workload identity and capability policy.
...
The same live hierarchy controls administrative startup scripts: a group MCP
identity can read or update its own script without supplying any target ID and
can use the explicit group tools only for its group or descendants. A platform
identity can manage any provisioned group workspace. Sibling, ancestor, unknown,
and unprovisioned targets return authorization-safe errors, and every script
operation carries a bounded server-side deadline.
