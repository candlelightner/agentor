...
A worker can be **exported** to a single portable v2 `.tar` bundle through durable owner-scoped export jobs (`POST /api/containers/:id/export-jobs`, status/cancel/download under `/api/export-jobs/:jobId`). Jobs persist under `users/<userId>/export-jobs.json`; completed artifacts are private streamed files with automatic expiry. Workspace-only is the default, while `rootfs.tar` is an explicit advanced option. It is the uncompressed `docker export` tar, preserved verbatim to avoid spending tens of minutes compressing and then decompressing a large immutable filesystem. That speed trade-off can require substantially more temporary and artifact storage, so the UI warns accordingly. The archive pipeline is abortable, queue-bounded, disk-preflighted, and reports phases/bytes. Bundles contain `manifest.json`, `workspace.tar.gz`, `agents.tar.gz`, and optional rootfs; account credentials, Kilo shared data/config, environment variable values, and domain basic-auth credentials are stripped. Import accepts both current v2 `rootfs.tar` bundles and legacy v1 `rootfs.tar.gz` bundles. **Import** (`ContainerManager.importWorker`) restores the bundle as a brand-new worker (fresh UUID): it resolves/recreates the non-secret environment definition, `docker import`s the rootfs into a per-worker image (replicating the standard image's entrypoint/env; falls back to the standard image on failure), creates the container stopped, restores the two volumes via `putArchive`, starts it, and recreates non-secret mappings. See @docs/production.md for the full flow.
