...
A background orphan sweeper (`server/utils/orphan-sweeper.ts`) runs at startup and every 10 minutes: it reads the auth DB's `user` table and, for any candidate `userId` whose row is gone, drains owner activity and removes live/archived workers under the shared owner→worker lifecycle fence before dropping the user from every in-memory store and recursively removing `<DATA_DIR>/users/<userId>/`. This takes workspaces, agent dirs, credentials, the per-user Kilo config + shared data dirs, and every per-user JSON file with it. Managed custom-volume records are first copied as detached records to the administrator-retained storage partition, so account cleanup cannot silently erase their Docker data; an administrator must remove those retained volumes explicitly. A runtime/storage cleanup failure defers the remaining deletion while preserving retryable worker identity; queued worker mutations revalidate the owner after the fence and cannot resurrect deleted state.
