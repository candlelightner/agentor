...
A single Docker image (`agentor-worker`, built from `worker/`) contains all agent CLIs and their setup scripts. **Credentials are scoped per user**: OAuth/subscription tokens live as JSON files at `<DATA_DIR>/users/<userId>/credentials/{claude,codex,gemini}.json` and are bind-mounted directly into every worker that user owns at the exact path each CLI reads and writes — `/home/agent/.agent-data/.claude/.credentials.json`, `/home/agent/.agent-data/.codex/auth.json`, `/home/agent/.agent-data/.gemini/oauth_creds.json`. Kilo's auth is handled differently: Kilo's **complete** `~/.local/share/kilo` data directory is shared per user — the orchestrator **directory**-binds `<DATA_DIR>/users/<userId>/kilo/data` at `/home/agent/.agent-data/.kilo/shared-data` (the entrypoint symlinks `~/.local/share/kilo → .agent-data/.kilo/shared-data`), so login (`auth.json`), provider API keys configured through Kilo's VS Code UI, and Kilo SQLite sessions/history are all shared per user across that user's workers. A directory bind (rather than a file bind on `auth.json`) is required because Kilo atomically temp+renames `auth.json`, which would break a file-level bind. Legacy `credentials/kilo.json` and per-worker `.kilo/data` migrate into the shared dir on the next worker start/rebuild (missing auth providers merge without overwriting shared entries; first available DB/history wins); Account **Reset** targets the live shared auth. Kilo is a fourth Account credential/reset row alongside Claude/Codex/Gemini, but it has no usage-monitoring endpoint and no separately installed Kilo CLI — the orchestrator only manages its shared config/auth/data files; no Kilo source is patched and no desktop VS Code package is installed. Since `~/.<agent>` is symlinked to `.agent-data/.<agent>/` by `worker/entrypoint.sh`, CLI writes land on the host file immediately and every other worker the same user owns sees the update without any restart. In directory mode the orchestrator pre-creates the mountpoint files under `<DATA_DIR>/users/<userId>/agents/<id>/.<agent>/` before starting the worker so Docker Desktop's virtiofs accepts the nested bind. Different users have completely isolated credentials. Env vars (API keys, the GitHub token, and any custom keys) are also per-user — managed from the dashboard's Account modal and persisted as a uniform key/value list in `<DATA_DIR>/users/<userId>/env-vars.json` (`{ envVars: [{ key, value }] }`, keyed by the env var NAME, no hardcoded fields). The SSH public key is kept separately at `<DATA_DIR>/users/<userId>/ssh/authorized_keys` (managed via `/api/account/ssh-key`, not part of env-vars.json). Copying OAuth tokens from a local machine is not supported because refresh token rotation would cause the local and worker tokens to go out of sync — always log in inside a worker.

**Kilo per-user config/data vs per-worker state/cache**: Kilo's global config (`~/.config/kilo`) is shared per user across all that user's workers/new workers — the orchestrator bind-mounts the per-user config directory `<DATA_DIR>/users/<userId>/kilo/config` over `/home/agent/.agent-data/.kilo/config` (the entrypoint symlinks `~/.config/kilo → .agent-data/.kilo/config`), so a config write through the canonical path on one worker is visible on every other worker the same user owns. Kilo's complete data directory (`~/.local/share/kilo`) is likewise shared per user — directory-bound from `<DATA_DIR>/users/<userId>/kilo/data` over `.agent-data/.kilo/shared-data` (the entrypoint symlinks `~/.local/share/kilo → .agent-data/.kilo/shared-data`), so login, provider API keys, and Kilo SQLite sessions/history are shared across all that user's workers. By contrast, Kilo **state** and **cache**, plus code-server user/UI state, stay **per-worker** in `.agent-data` (e.g. `~/.agent-data/.code-server`, `~/.agent-data/.kilo/state`, `~/.agent-data/.kilo/cache`) and survive restart/rebuild/archive — they are NOT shared across workers.

**Backup and recovery scope**: Normal worker exports and the default encrypted backup selection include `/workspace` plus a credential-filtered copy of per-worker `.agent-data`. Restoring such a bundle **into a new worker** recovers Claude, Codex, and Gemini configuration/history stored in that volume, together with per-worker Kilo state/cache and code-server state. OAuth files and the user-wide Kilo `config` and `data` directories are deliberately excluded: they are account-owned shared state and Kilo data can contain provider keys. To make an encrypted recovery copy of shared Kilo history/config, explicitly add `/home/agent/.agent-data/.kilo/shared-data` and/or `/home/agent/.agent-data/.kilo/config` in the backup path picker; restore that artifact into a **new** worker. Original-worker restore intentionally replaces `/workspace` only, and rejects backups containing explicit extra paths, so it never overwrites agent state or account-owned shared Kilo data.
...
**Runtime intent and recovery**: `desiredRuntimeStatus` is durable operator intent (`running` or `stopped`), separate from Docker's observed state. The API may report transient `starting`, `recovering`, or `unknown` states; `unknown` includes a safe, value-free retryable diagnostic when Docker cannot be verified. Startup reconciliation converges desired-running workers through Agentor and leaves desired-stopped workers stopped. It serializes with live lifecycle mutations so an in-flight create/rebuild/recovery is never misclassified as missing. Secret-bearing workers use Docker restart policy `no`, rather than daemon auto-restart, because Agentor must materialize their tmpfs secrets; managed restart has bounded bootstrap retries, and the runtime probe requires the root-owned tmpfs handshake marker before reporting them healthy. `POST /api/containers/:id/recover` (or MCP `workers.recover`) is the escalation for an unresponsive runtime: it verifies persistent mounts, replaces only disposable compute, restores managed bootstrap, and reconciles desired plugins. It never deletes workspace, agent-data, DinD, or selected persistent-path volumes. Legacy records without `desiredRuntimeStatus` are inferred only from a successfully verified observation; an old ambiguous/unverifiable runtime remains `unknown` rather than being guessed running or stopped.
...
When `GITHUB_TOKEN` is present, the entrypoint converges Git's GitHub credential-helper setting with `git config --replace-all`. This repairs persistent agent-data from older workers that accumulated duplicate helper values, is safe on every start, and never writes the token to Git configuration or logs.
...
0a. **Agent data symlinks** — if `/home/agent/.agent-data` is mounted, create symlinks from `~/.claude`, `~/.gemini`, `~/.codex`, `~/.agents`, `~/.claude.json`, and the Kilo XDG paths (`~/.config/kilo`, `~/.local/share/kilo`, `~/.local/state/kilo`, `~/.cache/kilo`) to volume subdirectories. Fixes ownership (`chown -R agent:agent`).
