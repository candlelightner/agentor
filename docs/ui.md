...
**Regular Editor iframe vs Persistent VS Code app:** the regular Editor pane (the `ServicePane` iframe embedding code-server) is a lightweight viewer. By Kilo's design, **disposing/closing Kilo's VS Code panel in the regular Editor iframe can abort active Kilo sessions**. For a long-lived editor that survives the viewer being closed, the Apps pane offers a **Persistent VS Code** singleton app: Apps → **Persistent VS Code** launches a Chromium **app-mode** client against the local code-server (already running on port 8443) under the existing Xvfb/noVNC display stack, with a persistent browser profile in the agent-data volume and no exposed port. The user then opens **Desktop** to view and drive it. Closing the external browser tab or the noVNC Desktop pane only disconnects the *viewer* — the Chromium client, the in-browser VS Code client, the Kilo panel, and any active Kilo agent work all keep running. It reuses code-server + the existing Chromium — no Kilo source is patched and no desktop VS Code package is installed. The persistent client still stops on an explicit app Stop, closing its X window on the desktop, worker stop/rebuild/archive/delete, a crash, or a VPS restart. It is not auto-started.
