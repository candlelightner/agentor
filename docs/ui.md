...
- `useClipboardPasteBridge.ts` (`useClipboardPaste`): on a real Ctrl/Cmd+V keydown, xterm's custom key handler (`useTerminal.ts`) synchronously swallows the key only when the async Clipboard API is available, then bridges asynchronously: image → normalise to PNG, POST to `POST /api/containers/:id/clipboard`, replay one raw Ctrl+V (`\x16`) over the terminal WebSocket (Codex/GUI apps read the synced X11 CLIPBOARD); text → xterm's normal `term.paste()` / bracketed-paste path (no X clipboard hit); unsupported/denied/error → replay the original key. Per-target concurrency guard; never logs contents; error toasts never include contents.
