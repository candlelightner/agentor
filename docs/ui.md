...
**Per-worker metrics** (`useWorkerMetrics`): the only resource metrics — there is no host/system card (host metrics are OS-dependent). Polled **once** in `AppSidebar` (one 10s `GET /api/worker-metrics`) and passed to each `ContainerCard` as a `metric` prop via `metricFor(id)` — so adding cards does not multiply pollers. The card renders a compact `data-testid="worker-metrics"` row (CPU% of host, RAM **used bytes**, durable Disk **used bytes**, ↓down/↑up) only while running. cpu/mem/net come from bounded `container.stats` calls (3s server cadence). Disk is sampled on a slower 60s cadence by a bounded in-worker `du` of `/workspace` plus agent data; Docker `SizeRw`, disposable rootfs changes, and DinD storage are intentionally excluded so a storage-driver walk cannot obstruct recovery. `formatBytes`/`formatRate` (`app/utils/format.ts`) format the values.

**Worker export**: the card's Export button opens a durable background-export modal. Starting an export quickly returns a job ID; the modal polls phase/progress, survives reloads, supports cancellation, and navigates to the authenticated download endpoint so the browser streams the completed artifact directly to disk. Workspace-only is the default. Container-root-filesystem capture is an explicitly warned advanced option because it can be large and slow.
