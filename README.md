...
- **Durable worker export/import** — background jobs with progress, cancellation, expiry, and browser-streamed downloads produce portable `.tar` bundles; workspace-only is fast/default and advanced rootfs capture uses an uncompressed Docker-export tar for speed, with legacy compressed-bundle import compatibility
